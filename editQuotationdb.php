...
if ($row1 = mysqli_fetch_array($result1)) {
  ...
  $table = array(array("Quotation Number","$invoice_code"));
$lineheight = 7;
$fontsize = 9;
...
$table = array(array("Quotation Date","$purchaseDate"));
$lineheight = 7;
$fontsize = 9;
$widths = array(47.25,47.25,47.25,47.25);
$aligns = array('L','L','L','L');
...
$pdf->SetFillColor(255, 255, 255); // Reset to white background for table content
 ...
 $delete_items_sql = "DELETE FROM quotation_items WHERE invoice_id=?";
...
$i=0;
error_log("Processing product: " . print_r($product, true), 3, "debug_products_insert.log");

foreach ($_POST["products"] as $key => $product)
 {
if (!isset($product['pname']) || empty(trim($product['pname']))) {
    file_put_contents("debug_issue.log", "Skipping product due to missing name: " . print_r($product, true), FILE_APPEND);
    continue;
}
    // if (empty($products_name)) {
    // error_log("Skipping product: " . print_r($product, true), 3, "debug_skipped.log");
    // continue;
    //     }
 ...
 $i++;
    ...
    $in_ex_gst = isset($product['pin_ex_gst']) ? mysqli_real_escape_string($conn, $product['pin_ex_gst']) : '';
        ...
        if ($priceval == 0 && $qtyvalue == 0 && $totalval == 0) {
        continue;
    }
    ...
    //   if (empty($products_name) || $priceval == 0) {
    //     continue;
    // }
...
$l1 = $line_tot + $cgstval + $sgstval + $igstval;
    ...
    // Insert query
   $sql = "INSERT INTO `quotation_items` (`invoice_id`, `itemno`,`product_id`, `product`, `prod_desc`, `qty`, `price`, `discount`,`line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`) 
                        VALUES ('$inv_id', '$i', '$productid','$products_name', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$l1', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst')";
...
$y = $pdf->GetY();
 ...
 // $sql = "INSERT INTO `quotation` (`id`, `invoice_code`, `customer_id`, `customer_name`, `customer_email`, `invoice_date`, `due_date`, `total_amount`, `total_gst`, `total_cess`, `grand_total`, `terms_condition`, `note`, `quotation_file`, `status`, `branch_id`, `created_by`) 
 //                VALUES ('$id', '$invoice_code', '$cst_mstr_id', '$customer_name', '$customer_email', '$purchaseDate', '$dueDate', '$final_taxable_amt', '$final_gst_amount', '$final_cess_amount', '$total_amount', '$terms', '$note', '$filename', 'Not Converted', '$branch_id', '$created_by')";
    ...
    $update_file_query = "UPDATE quotation SET quotation_file=?,invoice_date=?, due_date=?, total_amount=?, total_gst=?, total_cess=?,grand_total=?,terms_condition=?, note=?  WHERE id=?";
    $stmt_file = $conn->prepare($update_file_query);
    if (!$stmt_file) {
        die("Error preparing file update statement: " . $conn->error);
    }
    $stmt_file->bind_param("sssssssssi", $filename, $purchaseDate, $dueDate,$final_taxable_amt, $final_gst_amount, $final_cess_amount, $total_amount, $terms_condition, $note, $inv_id);
...
$sql_delete_charges = "DELETE FROM quotation_additional_charges WHERE invoice_id = ?";
...
if (isset($_POST['additionalCharges']['charge_type']) && isset($_POST['additionalCharges']['charge_price'])) {
    foreach ($_POST['additionalCharges']['charge_type'] as $key => $chargeType) {
        ...
        $sql_insert_charge = "INSERT INTO quotation_additional_charges (invoice_id, charge_type, charge_price, created_on)
                              VALUES (?, ?, ?, NOW())";
...
// $stmt_update_ledger->close();
    ...
    echo '<script>alert("Successfully Updated Quottaion");';
    echo 'window.location.href = "view-estimate-action.php?qid=' . $inv_id . '";</script>';
?>