                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h4 class="m-b-10">HSN Sales Report</h4>
                            </div>
                            <ul class="breadcrumb" style="float: right; margin-top:-40px;">
                                <li class="breadcrumb-item"><a href="index.php"><i class="feather icon-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="#">HSN Sales Report</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
...
<div class="card-body table-border-style">
             ...
             <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        // Assuming HSN/SAC is either in `hsn_code` or `SAC_Code`
                        echo "<tr>";
                        echo "<td>" . $row['hsn_code'] . "</td>";  // HSN/SAC
                        echo "<td>" . $row['units'] . "</td>";  // Unit
                        echo "<td>" . $row['gst_rate'] . "</td>";  // Tax Rate
                        
                        
                     if ($row['in_ex_gst'] === "exclusive of GST") {
    
    echo "<td>" . ($row['price'] * $row['gst_rate']) . "</td>";
} elseif ($row['in_ex_gst'] === "inclusive of GST") {
    ...
    echo "<td>" . $row['price'] . "</td>";
                        ...
                        echo "<td>" . $row['opening_stock'] . "</td>";  // Quantity (example)
                        // Calculate taxes (CGST, SGST, IGST, CESS)
                        $cgst = ($row['gst_rate'] / 2) * $row['net_price'];  // Example calculation for CGST
                        $sgst = ($row['gst_rate'] / 2) * $row['net_price'];  // Example calculation for SGST
                        $igst = 0;  // Example for IGST if not applicable
                        $cess = $row['cess_amt'];  // Assuming `cess_amt` is already provided
                        
                        
                        // echo "<td>" . $cgst . "</td>";  // CGST
                        // echo "<td>" . $sgst . "</td>";  // SGST
                        // echo "<td>" . $igst . "</td>";  // IGST
                        // echo "<td>" . $cess . "</td>";  // CESS
                        
                         echo "<td>" . $row['cgst'] . "</td>";  
                          echo "<td>" . $row['sgst'] . "</td>";  
                           echo "<td>" . $row['igst'] . "</td>";  
                            echo "<td>" . $row['cess_rate'] . "</td>";  
                            
                        $total_amount = $row['net_price'] + $cgst + $sgst + $igst + $cess;  // Total Amount calculation
                        
                         if ($row['in_ex_gst'] === "exclusive of GST") {
    
    echo "<td>" . ($row['price'] * $row['gst_rate'] *$row['balance_stock']) . "</td>";
    // echo "<td>" . $total_amount . "</td>";  // Total Amount
} elseif ($row['in_ex_gst'] === "inclusive of GST") {
    
    echo "<td>" . $row['price']  *$row['balance_stock']. "</td>";
    // echo "<td>" . $total_amount . "</td>";  // Total Amount
}
