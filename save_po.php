...
if (isset($_POST['submit'])) {
    ...
    $conn->begin_transaction();
        ...
        $result1 = mysqli_query($conn, "SELECT id FROM purchase_orders ORDER BY id DESC LIMIT 1");
...
$pdf->SetFont('Arial', '', 9);
 ...
 $result1 = mysqli_query($conn, "SELECT *  FROM customer_master JOIN address_master ON customer_master.id = address_master.customer_master_id WHERE contact_type = 'Supplier'");
...
$pdf->SetTextColor(0,0,0,0);
  ...
  $table = array(array("Purchase Order NO","$invoice_code"));
$lineheight = 7;
$fontsize = 9;
...
$table = array(array("Purchase Order Date","$invoice_date"));
$lineheight = 7;
$fontsize = 9;
$widths = array(47.25,47.25,47.25,47.25);
$aligns = array('L','L','L','L');
...
$table = array(array("Due Date","$due_date","Place of Supply","{$row1['s_state']}"));
$lineheight = 7;
$fontsize = 10;
$widths = array(47.25,47.25,47.25,47.25);
$aligns = array('L','L','L','L');
    ...
    foreach ($_POST["products"] as $key => $product) {
      ...
      $batch_no = $_POST['batch_no'][$key] ?? '';
        ...
        $pdf->Cell($widths[11], 8, number_format($line_tot, 2), 1, 1, 'C'); // Total column
           ...
           $sql = "INSERT INTO `po_items` (`invoice_id`, `itemno`, `product`, `prod_desc`, `qty`, `price`, `discount`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`, `batch_no`) 
                        VALUES ('$id', '$key', '$product', '$proddesc', '$qtyvalue', '$priceval', '$discountval', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$line_tot', '$in_ex_gst_val', '$batch_no')";

                if (!$conn->query($sql)) {
                    throw new Exception("Failed to insert quotation items: " . $conn->error);
                }
    }
}
...
file_put_contents($filename, $pdfdoc);
 ...
 // Insert main invoice
        $sql = "INSERT INTO `purchase_orders` (`id`, `invoice_code`, `customer_id`, `customer_name`, `customer_email`, `invoice_date`, `due_date`, `total_amount`, `total_gst`, `total_cess`, `grand_total`, `terms_condition`, `note`, `quotation_file`, `status`, `branch_id`, `created_by`) 
                VALUES ('$id', '$invoice_code', '$cst_mstr_id', '$customer_name', '$customer_email', '$invoice_date', '$due_date', '$final_taxable_amt', '$final_gst_amount', '$final_cess_amount', '$total_amount', '$terms', '$note', '$filename', 'Not Converted', '$branch_id', '$created_by')";

        if (!$conn->query($sql)) {
            throw new Exception("Failed to insert quotation: " . $conn->error);
        }
    ...
    foreach ($_POST['additionalCharges']['charge_type'] as $key => $chargeType) {
        ...
        $sql = "INSERT INTO po_additional_charges (invoice_id, charge_type, charge_price, created_on)
    ...
    $successMsg = "Purchase Order created successfully";
    echo "<script>
             alert(" . json_encode($successMsg) . ");
             window.location = 'view-purchase-order.php';
          </script>";
    ...
    $errorMsg = "Failed to create : " . $e->getMessage();
    echo "<script>
