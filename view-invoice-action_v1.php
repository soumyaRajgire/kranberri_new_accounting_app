...
function getQuotationDetails($conn, $inv_id) {
    ...
    // Query for fetching main quotation details
    ...
    if ($result->num_rows > 0) {
        // Fetch the main quotation details
        $quotationData = $result->fetch_assoc();

        // Now fetch the invoice items from the same result set or a separate query
        $quotationItems = [];
...
// Fetching quotation details for the specific invoice ID
$inv_id = $_GET['inv_id'];
...
<a href="#" id="myBtn" data-invoice-id="<?php echo $inv_id; ?>" class="btn border border-grey" data-toggle="modal" data-placement="top" title="Open Link">E-Invoice</a>
        ...
        {
            ?>
           <a href="#" data-toggle="modal" data-target="#gstCredentialsModal"   data-irn="<?php echo $quotationDetails['Irn']; ?>" class="btn border border-grey " data-toggle="tooltip" data-placement="top" title="Open Link">Cancel E-Invoice</a> 
           <?php
        }
...
<div id="shareModal" class="modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); justify-content: center; align-items: center;">
    <div class="modal-content" style="background: white; padding: 20px; border-radius: 8px; width: 400px; max-width: 90%;">
        ...
        <form id="emailForm" action="send_invoice_email.php" method="POST" enctype="multipart/form-data">
            ...
            <input type="email" class="form-control" id="customer_email1" name="customer_email1" value="<?php echo htmlspecialchars($invoiceDetails['customerEmail'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" required />
