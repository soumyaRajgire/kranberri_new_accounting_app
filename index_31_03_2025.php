...
<section class="pcoded-main-container">
    ...
    <?php
            ...
            $sql = "SELECT SUM(grand_total) AS total_invoice_amount 
                    FROM quotation 
            ...
            if ($result) {
                ...
                $grandTotal = isset($row['total_invoice_amount']) ? $row['total_invoice_amount'] : 0;

                // Display the grand_total value in the button
                echo '<a href="" class="btn btn-warning" style="float: right; margin-top: -60px;">INR ' . number_format($grandTotal, 2) . '</a>';
    ...
    <div class="card">
        <div class="card-body">
            ...
            if ($result) {
                ...
                // Display the grand_total value in the button
                echo '<a href="" class="btn btn-success" style="float: right; margin-top: -60px;">INR ' . number_format($grandTotal, 2) . '</a>';
...
<?php
          ...
          // SQL query to fetch the grand_total for the current month
          $sql = "SELECT SUM(total_amount) AS total_receipt_amount 
                  FROM receipts
          ...
          if ($result) {
              ...
              // Corrected variable reference
              $grandTotal = isset($row['total_receipt_amount']) ? $row['total_receipt_amount'] : 0;
        ...
        <?php
// Query to get total invoices for the current month
$sql_invoices = "SELECT SUM(grand_total) AS total_invoice_amount 
                 FROM invoice 
                 WHERE MONTH(invoice_date) = MONTH(CURRENT_DATE) 
                 AND YEAR(invoice_date) = YEAR(CURRENT_DATE)";
...
$total_invoices = isset($row_invoices['total_invoice_amount']) ? $row_invoices['total_invoice_amount'] : 0;

// Query to get total receipts for the current month
$sql_receipts = "SELECT SUM(total_amount) AS total_receipt_amount 
                 FROM receipts
                 WHERE MONTH(receipt_date) = MONTH(CURRENT_DATE) 
                 AND YEAR(receipt_date) = YEAR(CURRENT_DATE)";

$result_receipts = $conn->query($sql_receipts);
$row_receipts = $result_receipts->fetch_assoc();
$total_receipts = isset($row_receipts['total_receipt_amount']) ? $row_receipts['total_receipt_amount'] : 0;

// Calculate receivables
$total_receivables = $total_invoices - $total_receipts;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-text" style="margin-top: -15px;">Current Month</p>
            <a href="" class="btn btn-success" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($total_receivables, 2); ?>
            </a>
        </div>
    </div>
</div>
...
<?php
// Close connections
$result_invoices->close();
$result_receipts->close();
...
$total_unaccounted = $total_receipts - $total_invoices;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-text" style="margin-top: -15px;">Current Month</p>
            <a href="" class="btn btn-warning" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($total_unaccounted, 2); ?>
            </a>
        </div>
    </div>
</div>
...
// Query to fetch total GST collected for the current month
$sql_gst = "SELECT SUM(total_gst) AS gst_collected 
            FROM invoice 
            WHERE MONTH(invoice_date) = MONTH(CURRENT_DATE) 
            AND YEAR(invoice_date) = YEAR(CURRENT_DATE)";

$result_gst = $conn->query($sql_gst);
$row_gst = $result_gst->fetch_assoc();
$gst_collected = isset($row_gst['gst_collected']) ? $row_gst['gst_collected'] : 0;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-text" style="margin-top: -15px;">Current Month</p>
            <a href="" class="btn btn-success" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($gst_collected, 2); ?>
            </a>
        </div>
    </div>
</div>
...
// Query to fetch total Purchase Orders amount for the current month
$sql_po = "SELECT SUM(grand_total) AS total_purchase_orders 
...
$total_purchase_orders = isset($row_po['total_purchase_orders']) ? $row_po['total_purchase_orders'] : 0;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-text" style="margin-top: -15px;">Current Month</p>
            <a href="" class="btn btn-warning" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($total_purchase_orders, 2); ?>
            </a>
        </div>
    </div>
</div>
...
// Query to fetch total Purchase Invoices amount for the current month
$sql_pi = "SELECT SUM(grand_total) AS total_purchase_invoices 
           FROM pi_invoice 
           WHERE MONTH(invoice_date) = MONTH(CURRENT_DATE) 
           AND YEAR(invoice_date) = YEAR(CURRENT_DATE) ";  // Ensuring deleted invoices are excluded

$result_pi = $conn->query($sql_pi);
$row_pi = $result_pi->fetch_assoc();
$total_purchase_invoices = isset($row_pi['total_purchase_invoices']) ? $row_pi['total_purchase_invoices'] : 0;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-text" style="margin-top: -15px;">Current Month</p>
            <a href="" class="btn btn-success" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($total_purchase_invoices, 2); ?>
            </a>
        </div>
    </div>
</div>
...
$sql_payments = "SELECT SUM(paid_amount) AS total_payments 
                 FROM voucher 
                 WHERE MONTH(voucher_date) = MONTH(CURRENT_DATE) 
                 AND YEAR(voucher_date) = YEAR(CURRENT_DATE)";

$result_payments = $conn->query($sql_payments);
$row_payments = $result_payments->fetch_assoc();
$total_payments = isset($row_payments['total_payments']) ? $row_payments['total_payments'] : 0;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-text" style="margin-top: -15px;">Current Month</p>
            <a href="" class="btn btn-warning" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($total_payments, 2); ?>
            </a>
        </div>
    </div>
</div>
...
$sql_pi = "SELECT SUM(grand_total) AS total_purchase_invoices 
           FROM pi_invoice 
           WHERE MONTH(invoice_date) = MONTH(CURRENT_DATE) 
           AND YEAR(invoice_date) = YEAR(CURRENT_DATE)"; // Excluding deleted invoices
...
$account_payable = $total_purchase_invoices - $total_payments;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-text" style="margin-top: -15px;">Current Month</p>
            <a href="" class="btn btn-success" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($account_payable, 2); ?>
            </a>
        </div>
    </div>
</div>
...
?>
    ...
    <div class="row" style="margin-top: -20px;">
    <?php
// Query to fetch ITC Receivable (Total GST Paid on Purchases) for the last month
$sql_itc = "SELECT SUM(total_gst) AS itc_receivable 
            FROM pi_invoice 
            WHERE MONTH(invoice_date) = MONTH(CURRENT_DATE) - 1 
            AND YEAR(invoice_date) = YEAR(CURRENT_DATE)"; // Exclude deleted invoices

$result_itc = $conn->query($sql_itc);
$row_itc = $result_itc->fetch_assoc();
$itc_receivable = isset($row_itc['itc_receivable']) ? $row_itc['itc_receivable'] : 0;
?>
...
<div class="col-md-6">
    <div class="card">
        <div class="card-body">
            ...
            <a href="itc-details.php" class="btn btn-warning" style="float: right; margin-top: -60px;">
                INR <?php echo number_format($itc_receivable, 2); ?>
            </a>
        </div>
    </div>
</div>
