...
<link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.css" />
    ...
    if (inclusiveGst === "inclusive of GST" && price > 0) {
        taxablePrice = nonTaxable > 0
            ? (price - nonTaxable) / (1 + gstRate / 100)
            : price / (1 + gstRate / 100);
    } else if (inclusiveGst === "exclusive of GST" && price > 0) {
        taxablePrice = nonTaxable > 0 ? price - nonTaxable : price;
    } else {
        return null; // Invalid price or GST type
    }
...
function calculateBatchRow(element) {
    const $row = $(element).closest(".batch-row");

    const price = parseFloat($row.find(".price-input").val()) || 0;
    const gstRate = parseFloat($row.find(".gst-rate-input").val()) || 0;
    const inclusiveGst = $row.find(".inclusive-gst-select").val();
    const nonTaxable = parseFloat($row.find(".non-taxable-input").val()) || 0;
    const cessRate = parseFloat($row.find(".cess-amt-input").val()) || 0;

    const netPriceField = $row.find(".net-price-input");
    const cessAmountField = $row.find(".cess_amount-input");

    const result = computePriceValues({ price, gstRate, inclusiveGst, nonTaxable, cessRate });

    if (result) {
        netPriceField.val(`${result.taxablePrice} | ${result.gstAmount}`);
        cessAmountField.val(result.cessAmount);
    } else {
        netPriceField.val("");
        cessAmountField.val("");
    }
...
function calculatePrices(modalId) {
    const price = parseFloat($(".price-input[data-modal='" + modalId + "']").val()) || 0;
    const gstRate = parseFloat($(".gst-rate-input[data-modal='" + modalId + "']").val()) || 0;
    const inclusiveGst = $(".inclusive-gst-select[data-modal='" + modalId + "']").val();
    const nonTaxable = parseFloat($(".non-taxable-input[data-modal='" + modalId + "']").val()) || 0;
    const cessRate = parseFloat($(".cess-amt-input[data-modal='" + modalId + "']").val()) || 0;

    const netPriceField = $(".net-price-input[data-modal='" + modalId + "']");
    const cessAmountField = $(".cess_amount-input[data-modal='" + modalId + "']");
