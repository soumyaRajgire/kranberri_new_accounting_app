...
<div class="row" id="customer_data"></div>
  ...
  <datalist name="product" id="product">
    ...
    if ($result->num_rows > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
         echo '<option value="' . htmlspecialchars($row["name"]) . '" data-productid="' . $row["id"] . '" data-rawmatid="' . $row["raw_mat_id"] . '"></option>';
  ...
  <in name="productid" id="productid" hidden />
<input type="hidden" name="raw_mat_id" id="raw_mat_id">
...
function add_more() {
  ...
  // console.log(productid);
    ...
    // const batchid = parseFloat($('#hidden_batch_no').val()) || 0; 
       const batchNo = ($('#batchSelect').val()) || 0; 
       // console.log("bathcno",batchNo);
// console.log(batchid);
 ...
 // Check if the product already exists in the table
   ...
   // Calculate taxable amount and GST based on inclusive/exclusive GST
    if (in_ex_gst === "inclusive of GST") {
        ...
        console.log("from inclusive of gst",gstAmount);
        ...
        gstAmount = taxableAmount * (gst / 100); // GST amount
        console.log("from exclusive of gst",gstAmount);
         console.log("taxable amount",taxableAmount);
...
const finalCessAmount = discountedTaxableAmount * (cess_rate / 100); 
    ...
    const rowHtml = `
        ...
        <input type="hidden" id="batch_no_val_${itemno}" name="batch_no_val[]" value="${batchNo}">
      <input type="hidden" id="raw_mat_val_${itemno}" name="raw_mat_val[]" value="${rawmatid}">
        </tr>
    `;
...
document.getElementById("cst_mstr_id").value = customerId;
  ...
  <script type="text/javascript">
document.getElementById('product_choice').addEventListener('change', function () {
    ...
    options.forEach(function (option) {
        if (option.value === inputVal) {
            ...
            rawMatId = option.getAttribute('data-rawmatid');
        }
    });

    // document.getElementById('productid').value = productId;
    document.getElementById('raw_mat_id').value = rawMatId;
});
