    ...
    <!--adding product -->
      <div class="row border-dark border-right border-left border-top border-bottom" id="box_loop_1">
          ...
          <div class="col-md-3 p-1 border-right border-left border-bottom">
            ...
            <th id="cess-th" style="display: none">CESS </th>
...
function add_more() {
    ...
    const discount = parseFloat($('#discount').val()) || 0; // Discount (default 0 if empty)
    const cess_rate = parseFloat($('#cess_rate').val()) || 0; // Cess rate (if applicable)
    ...
    const discountedPrice = price - (price * discount) / 100; // Price after discount
    const taxableAmount = discountedPrice * qty; // Total taxable amount for this row
    ...
    const cessAmount = (gstAmount * cess_rate) / 100;

    // Calculate Total Amount (including GST and Cess)
    ...
    const rowHtml = `
        <tr>
            ...
            <td>${cessAmount > 0 ? cessAmount.toFixed(2) : '-'}</td>
...
function calculate_totals() {
    ...
    $('#item-list tbody tr').each(function () {
        const row = $(this);
        const taxableAmount = parseFloat(row.find('td:nth-child(6)').text()) || 0; // Taxable Amount
        const cgst = parseFloat(row.find('td:nth-child(7)').text()) || 0; // CGST
        const sgst = parseFloat(row.find('td:nth-child(8)').text()) || 0; // SGST
        const igst = parseFloat(row.find('td:nth-child(9)').text()) || 0; // IGST
        const cessAmount = parseFloat(row.find('td:nth-child(10)').text()) || 0; // Cess Amount
        const totalAmount = parseFloat(row.find('td:nth-child(11)').text()) || 0; // Total Amount
    ...
    // Update individual GST fields if necessary
    $('#final_cgst').text(totalCGST.toFixed(2));
    $('#final_sgst').text(totalSGST.toFixed(2));
    $('#final_igst').text(totalIGST.toFixed(2));
}
...
function addCharge() {
    ...
    if (selectedOption.value) {
        ...
        row.innerHTML = `
        <div id="charge_${selectedOption.value}" class="additional-charge-row">
            <div class="row align-items-center">
                ...
                <div class="col-5">
                    ...
                    oninput="calc_total()"> <!-- Update total dynamically when input changes -->
                </div>
            </div>
        </div>
        `;
