...
<div id="deductStockModal" class="modal fade" tabindex="-1" aria-labelledby="deductStockModalLabel" aria-hidden="true">
    ...
    <?php include("productsModal-new.php");?>
...
function calculateBatchRow(inputField) {
    const batchRow = inputField.closest('.batch-row');

    // Batch-specific fields
    const priceField = batchRow.querySelector('input[name="batch_price[]"]');
    const netPriceField = batchRow.querySelector('input[name="batch_net_price[]"]');
    const cessAmountField = batchRow.querySelector('input[name="batch_cess_amount[]"]');

    // Get values from the main form
    const gstRateRaw = document.getElementById('gst_rate')?.value?.trim();
    const gstType = document.getElementById('inclusive_gst')?.value?.toLowerCase();
    const cessRate = parseFloat(document.getElementById('cess_rate1')?.value) || 0;

    if (!priceField || !gstRateRaw || !gstType) return;
    ...
    // Calculate GST & Net Price for the batch row
    ...
    // Set the values for the batch row
...
function recalculateBatchRows() {
    ...
    batchRows.forEach(row => {
        ...
        if (priceField) {
            calculateBatchRow(priceField); // Recalculate for each batch row
        }
    });
}

// Add event listeners to the common fields
// Add event listeners to the common fields in the main form
document.getElementById('inclusive_gst')?.addEventListener('change', recalculateBatchRows);
document.getElementById('gst_rate')?.addEventListener('change', recalculateBatchRows);
document.getElementById('cess_rate1')?.addEventListener('input', recalculateBatchRows);


function calculatePrices(context = 'products') {
    const modal = document.querySelector(`[data-modal="${context}"]`)?.closest('form') || document;
    
    // Get the main form's values
    ...
    // Calculate net price and GST for the main form
    let taxableAmount = price;
    let gstAmount = 0;
    ...
    // Now, trigger the recalculation for batch rows
    recalculateBatchRows(); // Ensure batch rows are recalculated
}
