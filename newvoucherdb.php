...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $customer_id = mysqli_real_escape_string($conn, $_POST['cst_mstr_id']);

     $reconciliation_option = $_POST['reconciliation_option'];
   ...
   $reconcile_amounts = $_POST['reconcile_amount'] ?? []; // Array of amounts allocated to invoices
    $total_reconciled = array_sum($reconcile_amounts);


  $total_amount = $amount; // Total receipt amount
    // $invoice_id = NULL; // Default invoice_id for receipts table
    // $reconcile_amounts = $_POST['reconcile_amount'] ?? []; // Array of reconciled amounts per invoice

    // Validate total reconciled amount matches the total payment amount
  if( $reconciliation_option  === "Custom Reconcile")
    if ($total_reconciled != $amount) {
        echo "<script>alert('Reconciled amount does not match the total amount.'); window.history.back();</script>";
        exit();
    }

    // $total_amount = mysqli_real_escape_string($conn,$_POST['grand_total']);
    // Fetch previously paid amount from receipts table
    // $res1 = mysqli_query($conn, "SELECT SUM(paid_amount) AS paid_amount FROM receipts WHERE invoice_id = '$invoice_pid'");
    ...
    $file_name = 'Voucher-'. $receipt_no .'.pdf';
...
if ($row1 = mysqli_fetch_array($result1)) {
    ...
    $pdf->Cell(0, 6, "VOUCHER", 'TR', 1, 'L');
     $pdf->Cell(140, 6, "{$row1['address']}", 'L', 0, 'L');
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(0, 6, "Voucher No.#: $receipt_no", 'R', 1, 'L');
    $pdf->SetFont('Arial', '', 9);
    $pdf->Cell(140, 6, "Phone: 91 7550705070", 'L', 0, 'L');
    $pdf->Cell(0, 6, "Date : $receipt_date", 'R', 1, 'L');
    ...
    $pdf->SetFont("Arial", "B", 8);
    $pdf->MultiCell(90, 6, "\nFor KRIKA MKB CORPORATION PRIVATE LIMITED \n\n\n Authorised Signatory\n", 0, 'R');
    $endYSecond = $pdf->GetY();
...
// Determine invoice_id and reconciliation status
if ($reconciliation_option == "Custom Reconcile" && !empty($reconcile_amounts)) {
    $reconciliation_status = "Partially Reconciled";
    $invoice_id = NULL; // Multiple invoices, so invoice_id is NULL
} elseif (!empty($reconcile_amounts) && count($reconcile_amounts) === 1) {
    $reconciliation_status = "Fully Reconciled";
    $invoice_id = array_key_first($reconcile_amounts); // Single invoice ID
} else {
    $reconciliation_status = "Not Reconciled";
    $invoice_id = NULL;
}
    ...
    $sql = "INSERT INTO voucher (
        `voucher_id`, `voucher_date`, `customer_id`, `invoice_id`, 
        `total_amount`, `paid_amount`, `pdf_file_path`, 
        `payment_mode`, `collected_by`, `bank_name`, `transactionid`, 
        `cheque_no`, `ddno`, `card_last_no`, `transaction_date`, `reconciliation_status`, `notes`, `created_at`,`branch_id`
    ) VALUES (
        '$receipt_no', '$receipt_date', '$customer_id', " . ($invoice_id ? "'$invoice_id'" : "NULL") . ", 
        '$total_amount', '$amount', '$filename', 
        '$payment_mode', '$collected_by', '$bank_name', '$transaction_no', 
        '$cheque_no', '$dd_no', '$card_last_no', '$transaction_date','$reconciliation_status', 
        '$notes', NOW(),'$branch_id')";
...
$receipt_id = mysqli_insert_id($conn);
     ...
     $sql_ledger = "INSERT INTO `ledger` (`voucher_id`, `transaction_date`, `transaction_type`, `account_id`, `account_name`, `amount`, `debit_credit`, `receipt_or_voucher_no`,`branch_id`)value('$receipt_id','$receipt_date','Voucher','$customer_id', '$customer_name','$total_amount','D','$receipt_no','$branch_id')";
        ...
        if (!$conn->query($sql_ledger)) {
         ...
         if ($reconciliation_option === "Custom Reconcile") {
         foreach ($reconcile_amounts as $invoice_id => $reconciled_amount) {
            $invoice_id = intval($invoice_id);
            $reconciled_amount = floatval($reconciled_amount);

            // Insert into reconciliation table
            $reconcile_query = "INSERT INTO voucher_reconciliation (voucher_id, pi_invoice_id, reconciled_amount, reconciliation_date
                                ) VALUES ($receipt_id, $invoice_id, $reconciled_amount, NOW() )";
            $conn->query($reconcile_query);

           // Dynamically calculate the total paid amount for the invoice
    $paid_query = " SELECT COALESCE(SUM(paid_amount), 0) AS total_paid FROM voucher WHERE voucher_id = $invoice_id";
    $paid_result = $conn->query($paid_query);
    $row = $paid_result->fetch_assoc();
    $total_paid = floatval($row['total_paid']) + $reconciled_amount; // Add the current reconciled amount

    // Calculate the due amount
    $due_amount_query = " SELECT grand_total FROM pi_invoice WHERE id = $invoice_id
    ";
    $due_result = $conn->query($due_amount_query);
    $row = $due_result->fetch_assoc();
    $grand_total = floatval($row['grand_total']);
    $due_amount = $grand_total - $total_paid;

    // Determine the status
    $status = ($due_amount == 0) ? 'Paid' : (($due_amount < $grand_total) ? 'Partial' : 'Pending');

    // Update the invoice table
    $update_invoice_query = "UPDATE pi_invoice  SET due_amount = $due_amount, status = '$status' WHERE id = $invoice_id";
            $conn->query($update_invoice_query);
        }

}
  echo "<script>
                alert('Receipt and reconciliation processed successfully.');
                window.location.href = 'view-voucher-action.php?voucherId={$receipt_id}';
              </script>";
    } else {
        echo "<script>
