...
<a href="<?php echo $quotationDetails['quotation_file']?>" id="pdfLink" style="display: none;">Download PDF</a>
...
<div class="btn-group">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink" style="margin-right: 55px;">
    <a class="dropdown-item" href="editQuotation.php?inv_id=<?php echo $inv_id?>">Edit Quotation</a>
        <a class="dropdown-item" href="convert-quotation-into-invoice.php?id=<?php echo $inv_id?>">Convert to Invoice</a>
...
?>
    ...
    <div class="modal-content" style="background: white; padding: 20px; border-radius: 8px; width: 400px; max-width: 90%;">
        ...
        <form id="emailForm" action="send_quotation_email.php" method="POST" enctype="multipart/form-data">
            <!-- Hidden Fields -->
            <input type="hidden" name="quotation_file" id="quotation_file" value="<?php echo isset($quotationDetails['quotation_file']) ? htmlspecialchars($quotationDetails['quotation_file'], ENT_QUOTES, 'UTF-8') : ''; ?>" />

            <!-- Visible Inputs -->
            <label for="email">To:</label>
            <input type="email" class="form-control" id="email" name="email" value="<?php echo isset($quotationDetails['customerEmail']) ? htmlspecialchars($quotationDetails['customerEmail'], ENT_QUOTES, 'UTF-8') : ''; ?>" required />

            <label for="subject">Subject:</label>
            <input type="text" class="form-control" id="subject" name="subject" value="Quotation Invoice #<?php echo htmlspecialchars($inv_id, ENT_QUOTES, 'UTF-8'); ?>" required />

            <label for="message">Message:</label>
            <textarea class="form-control" id="message" name="message" rows="4" required>
Dear Customer, please find attached the quotation invoice.
            </textarea>

            <input type="hidden" name="invoiceId" id="invoiceId" value="<?php echo htmlspecialchars($_GET['inv_id'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" />
...
<div class="row">
    <div class="col-md-8">
        ...
        <canvas id="pdf-canvas" style="width:800px"></canvas>
           <div id="controls">
...
<script>
   ...
   document.addEventListener("DOMContentLoaded", () => {
    ...
    const invoiceId = getQueryParam("qid"); // Extract invoice_id from URL

    // Fetch the dynamic PDF path from the backend
    fetch(`fetch_quotation_path.php?invoice_id=${invoiceId}`)
...
<script>
    ...
    $('#shareWhatsApp').on('click', function() {
        var pdfLink = $('#pdfLink').attr('href'); // Get the PDF file link

        if (pdfLink) {
            var message = "Hi, please find the quotation attached: " + pdfLink;
