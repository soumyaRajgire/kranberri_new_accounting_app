...
$total_amount1 = mysqli_real_escape_string($conn, $_POST['total_amount']);
...
if ($customer_name != "") {
    // $result1 = mysqli_query($conn, "SELECT MAX(id) as max_id FROM credit_note");
    // if (!$result1) {
...
foreach ($_POST['products'] as $productData) {
 ...
 $gst_amount = floatval($productData['gst_amt']);
    ...
    $sql_item = "
        INSERT INTO credit_note_items (
            cnote_id, productid,product, prod_desc, qty, price, line_total, gst, gst_amt, 
            cgst, sgst, igst, cess_rate, cess_amt,discount, total, created_by
        ) VALUES (
            '$cid', '$productid','$productName', '$productDescription', '$quantity', '$price', 
            '$line_total', '$gst', '$gst_amount', '{$productData['cgst']}', 
    ...
    if ($conn->query($sql_item) === TRUE) {
        echo "Credit note item added successfully.";
    } else {
        error_log("Error adding credit note item: " . $conn->error);
        echo "Error adding credit note item.";
    }
...
$transaction_type = "Credit Note"; // This can be changed based on the transaction type
$remark = "Credit Note"; // Can be customized
...
//     throw new Exception("Failed to generate PDF");
// }
$sql = "INSERT INTO credit_note (id,cnote_code, cnote_file, invoice_id, customer_id, branch_id, customer_name, email, cnote_date, total_amount, total_gst_amount, total_cess_amount, adjusted_amount, terms_condition, note, status, created_by) 
        VALUES ('$cid','$dnote_code', '$filename', '$purchase_invoice_id', '$cst_mstr_id', '$branch_id', '$customer_name', '$customer_email', '$dnoteDate', '$grand_total1', '$total_gst1', '$total_cess1', '$adjusted_amount', '$terms', '$note', 'pending', '$created_by')";
if (!$conn->query($sql)) {
    throw new Exception("Error Inserting Credit Note: " . $conn->error);
}
 //$cid = mysqli_insert_id($conn);


$sql_ledger = "INSERT INTO `ledger` (`voucher_id`, `transaction_date`, `transaction_type`, `account_id`, `account_name`, `amount`, `debit_credit`, `receipt_or_voucher_no`,`branch_id`)value($cid,'$dnoteDate','Credit Note','$cst_mstr_id', '$customer_name','$grand_total1','C','$dnote_code','branch_id')";
        ...
        $conn->commit();
       echo "<script>alert('Credit Note created successfully!'); window.location.href = 'manage-creditnote.php';</script>";
//         echo "<script>
//     Swal.fire({
//         icon: 'success',
//         title: 'Success',
