...
if (isset($_POST['submit'])) {
 ...
 // Generate PDF using FPDF
   ...
   $func = function($text, $c_width) {
    if (is_null($text)) {
        $text = ""; // Set default empty string if null
    }

    $len = strlen($text);
    $twidth = $this->GetStringWidth($text);
    $split = ($twidth != 0) ? floor($c_width * $len / $twidth) : 1; // Avoid division by zero
...
$file_name = "Quotation-".$invoice_code.'.pdf';
...
$l1 = $line_tot + $cgstval + $sgstval + $igstval;
  ...
  $pdf->Cell($widths[2], 8, number_format($priceval, 2), 1, 0, 'C');
        ...
        $pdf->Cell($widths[11], 8, number_format($l1, 2), 1, 1, 'C'); // Total column
         ...
         // $sql = "INSERT INTO `quotation_items` (`invoice_id`, `itemno`,`productid`, `product`, `prod_desc`, `qty`, `price`, `discount`,`line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`) 
         //                VALUES ('$id', '$key', '$productids','$product', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$l1', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst_val')";

         //        if (!$conn->query($sql)) {
         //            throw new Exception("Failed to insert invoice items: " . $conn->error);
         //        }

        $sql = "INSERT INTO `quotation_items` (`invoice_id`, `itemno`,`product_id`, `product`, `prod_desc`, `qty`, `price`, `discount`, `line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`, `batch_no`) 
VALUES ('$id', '$itemno','$productids', '$product', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$l1', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst_val', '$batch_no')";
...
file_put_contents($filename, $pdfdoc);
 ...
 // Insert main invoice
    ...
    foreach ($_POST['additionalCharges']['charge_type'] as $key => $chargeType) {
        ...
        $sql = "INSERT INTO quotation_additional_charges (invoice_id, charge_type, charge_price, created_on)
    ...
    $cst_mstr_id = isset($cst_mstr_id) ? $cst_mstr_id : $customer_id;
                    $purchaseDate = isset($purchaseDate) ? $purchaseDate : $transaction_date;
                    ...
                    include 'whatsapp_communication2.php';
                            ...
                            $pdfType = "Quotation";
                            $destinationURL="view-quotation.php";
       ...
       echo "<script>alert('Quotation created successfully'); window.location = 'view-quotation.php';</script>";
    } catch (Exception $e) {
