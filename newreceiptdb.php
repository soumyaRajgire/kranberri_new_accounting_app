...
if ($reconciliation_option == "Custom Reconcile" && !empty($reconcile_amounts)) {
    ...
    $sql = "INSERT INTO receipts (
        `recpt_id`, `receipt_date`, `customer_id`, `invoice_id`, 
        ...
        '$total_amount', '$amount', '$filename', '$payment_mode', '$collected_by', '$bank_name', '$transaction_no', 
        '$cheque_no', '$dd_no', '$card_last_no', '$transaction_date','$reconciliation_status', '$notes', NOW(),'$branch_id' )";

if (mysqli_query($conn, $sql)) {

      $receipt_id = mysqli_insert_id($conn);
        $sql_ledger = "INSERT INTO `ledger` (`voucher_id`, `transaction_date`, `transaction_type`, `account_id`, `account_name`, `amount`, `debit_credit`, `receipt_or_voucher_no`,`branch_id`)value('$receipt_id','$receipt_date','Receipt','$customer_id', '$customer_name','$total_amount','C','$receipt_no','$branch_id')";
        ...
        if (!$conn->query($sql_ledger)) {
         ...
         foreach ($reconcile_amounts as $invoice_id => $reconciled_amount) {
            ...
            $reconcile_query = "INSERT INTO reconciliation (
                                    receipt_id, invoice_id, reconciled_amount, reconciliation_date
                                ) VALUES (
                                    $receipt_id, $invoice_id, $reconciled_amount, NOW()
                                )";
