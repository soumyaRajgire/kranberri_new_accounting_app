...
<body class="">
    ...
    $query = "SELECT q.*, c.*, a.*, qi.*
              ...
              JOIN pi_invoice_items qi ON q.id = qi.invoice_id
...
// $conn->close();
    ...
    <?php include("createVoucherModal.php");?>
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h4 class="m-b-10">View Purchase Invoice</h4>
                        </div>
        ...
        <div class="col-md-6 col-lg-6">
            <ul class="nav">
                <li class="nav-item mt-2">
                    <h5><a href="#" class="customer_est_name text-primary"  style="font-size: 19px;">Supplier: <?php echo $quotationDetails['customerName'];?></a></h5>
                </li>
            </ul>
        </div>
...
{
 ?>
   <a href="#" data-toggle="modal" data-target="#voucherModal"  class="btn border border-grey d-none" data-toggle="tooltip" data-placement="top" title="Open Link">Voucher</a>
...
{
    ?>
<a href="#" data-toggle="modal" data-target="#voucherModal"  class="btn border border-grey " data-toggle="tooltip" data-placement="top" title="Open Link">Voucher</a>
...
if (is_array($quotationDetails) && isset($quotationDetails['status'])) {
    ...
    $query1 = "SELECT COUNT(*) as receipt_count FROM voucher WHERE invoice_id = '$inv_id'";
    $result1 = $conn->query($query1);
    $receiptData = $result1->fetch_assoc();
    $receiptCount = $receiptData['receipt_count'];

    // ✅ Check if a Credit Note exists for this invoice
    $query2 = "SELECT COUNT(*) as credit_note_count FROM debit_note WHERE purchase_invoice_id = '$inv_id'";
    ...
    // - A credit note is generated for the invoice
    $disableEdit = ($invoiceStatus == 'paid' || $invoiceStatus == 'partial' || $creditNoteCount > 0);
    ...
    if  (!$disableEdit) {
        echo '<a class="dropdown-item" href="update_purchase_invoice.php?inv_id=' . $invId . '">Edit Invoice</a>';
    } else {
        echo '<a class="dropdown-item text-muted disabled" href="#">Edit Disabled</a>';
    }

    if (!$disableDelete) {
        echo '<a class="dropdown-item text-danger" href="delete_pinvoice.php?inv_id=' . $invId . '" onclick="return confirm(\'Are you sure you want to delete this invoice?\');">Delete</a>';
    } else {
...
?>
   <!--  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink" style="margin-right: 55px;">
    <a class="dropdown-item" href="update_purchase_invoice.php?inv_id=<?php echo $inv_id; ?>">Edit Purchase Invoice</a>

        <a class="dropdown-item" href="delete_pinvoice.php?inv_id=<?php echo $inv_id; ?>" onclick="return confirm('Are you sure you want to delete this invoice?');">Delete</a>-->
...
// Fetch credit note adjustments for this invoice
    $query2 = "SELECT SUM(total_amount) AS total_credit_adjusted, dnote_date FROM debit_note 
               WHERE purchase_invoice_id = '$inv_id' AND is_deleted = 0";
    $result2 = $conn->query($query2);
...
<script>
   ...
   document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(`fetch_pinvoice_path.php?invoice_id=${invoiceId}`)
