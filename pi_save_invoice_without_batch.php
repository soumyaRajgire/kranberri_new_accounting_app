...
$l1 = $line_tot + $cgstval + $sgstval + $igstval;
        ...
        $pdf->Cell($widths[11], 8, number_format($l1, 2), 1, 1, 'C'); // Total column
           ...
           $sql = "INSERT INTO `pi_invoice_items` (`invoice_id`, `itemno`, `productid`, `product`, `prod_desc`, `qty`, `price`, `discount`, `line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`) 
           VALUES ('$id', '$key', '$productid', '$product', '$proddesc', '$qtyvalue', '$priceval', '$discountval', '$l1', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$total_amount')";
...
$stmt_st_master->bind_param("isissi", $productid,$invoice_code, $qtyvalue, $transaction_type, $remark, $created_by);
 ...
 // Insert main invoice
    ...
    foreach ($_POST['additionalCharges']['charge_type'] as $key => $chargeType) {
     ...
     $sql_ledger = "INSERT INTO `ledger` (`voucher_id`, `transaction_date`, `transaction_type`, `account_id`, `account_name`, `amount`, `debit_credit`, `receipt_or_voucher_no`,`branch_id`)value($id,'$purchaseDate','Purchase','$cst_mstr_id', '$customer_name','$total_amount','C','$invoice_code','branch_id')";
        ...
        echo "<script>alert('Purchase Invoice created successfully'); window.location = 'view-purchase-invoices.php';</script>";
    } catch (Exception $e) {
