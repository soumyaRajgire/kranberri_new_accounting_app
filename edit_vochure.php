...
function getVoucherDetails($conn, $voucherId)
{
    ...
    $query = "SELECT r.*, c.customerName, c.email, a.*
              FROM voucher r
...
$voucherDetails = getVoucherDetails($conn, $voucher_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $transaction_proof_path = $voucherDetails['trans_proof_doc']; // Keep old path by default

    // Handle file upload if a new file is provided
    if (!empty($_FILES['transaction_proof']['name']) && $_FILES['transaction_proof']['error'] === UPLOAD_ERR_OK) {
        $upload_dir = 'uploads/';
        if (!is_dir($upload_dir)) {
            mkdir($upload_dir, 0777, true);
        }

        $file_name = uniqid() . "_" . basename($_FILES['transaction_proof']['name']);
    ...
    // Fetch original invoice_id from the voucher
    $invoice_id = $voucherDetails['invoice_id'] ?? null;

    // Fetch total amount from invoice
    $invoice_query = "SELECT grand_total FROM pi_invoice WHERE id = ?";
    ...
    $total_amount = $invoice_row['grand_total'] ?? 0;
    $stmt_invoice->close();

    // Fetch total paid amount before update
    $res1 = mysqli_query($conn, "SELECT SUM(paid_amount) AS paid_amount FROM voucher WHERE invoice_id = '$invoice_id'");
    $row2 = mysqli_fetch_assoc($res1);
    $previous_paid_amount = $row2['paid_amount'] ?? 0;

    // Calculate the new due amount
    $updated_paid_amount = ($previous_paid_amount - $voucherDetails['paid_amount']) + $paid_amount;
    ...
    if ($transaction_proof_path !== $voucherDetails['trans_proof_doc']) {
    ...
    // Update Ledger
    $ledger_query = "UPDATE ledger SET 
    transaction_date = ?, 
    account_id = ?, 
    account_name = ?, 
    amount = ?, 
    debit_credit = 'D'
WHERE voucher_id = ? AND transaction_type = 'Voucher Payment'";

$stmt_ledger = $conn->prepare($ledger_query);
$stmt_ledger->bind_param("sisdi", $transaction_date, $customer_id, $voucherDetails['customerName'], $paid_amount, $voucher_id);

    if (!$stmt_ledger->execute()) {
        echo "Error updating ledger: " . $stmt_ledger->error;
        error_log("Exception: " . $stmt_ledger->error, 3, "error_log.txt");
        echo "<script>alert('error Ledger updating!')</script>";
    }
    else {
        error_log("Ledger Updated sucessfully: " , 3, "error_log.txt");
        echo "<script>alert('Ledger updated successfully!')</script>";
    }
    $stmt_ledger->close();

    // Update Voucher Reconciliation
    $reconcile_query = "UPDATE voucher_reconciliation SET 
    reconciled_amount = ?, 
    reconciliation_date = NOW() 
WHERE voucher_id = ? AND EXISTS 
    (SELECT 1 FROM voucher WHERE id = ?)";

    
    $stmt_reconcile = $conn->prepare($reconcile_query);
    $stmt_reconcile->bind_param("dii", $paid_amount, $voucher_id, $voucher_id);


    if (!$stmt_reconcile->execute()) {
        echo "Error updating voucher reconciliation: " . $stmt_reconcile->error;
    }
    $stmt_reconcile->close();

    // Update Invoice Status
    $invoice_update_query = "UPDATE pi_invoice SET due_amount = ?, status = ? WHERE id = ? AND EXISTS 
    (SELECT 1 FROM voucher WHERE invoice_id = pi_invoice.id AND id = ?)";

    $stmt_invoice_update = $conn->prepare($invoice_update_query);
    $stmt_invoice_update->bind_param("dsii", $due_amount, $status, $invoice_id, $voucher_id);

    if (!$stmt_invoice_update->execute()) {
        echo "Error updating invoice status: " . $stmt_invoice_update->error;
    }
    $stmt_invoice_update->close();

    echo "<script>alert('Voucher updated successfully!'); window.location.href='manage-voucher.php';</script>";
}
?>
