...
// Fetch only Party Wise Sales reports
$files = array_filter(scandir($directory, SCANDIR_SORT_DESCENDING), function($file) use ($branch_id) {
    return strpos($file, "Party_wise_Sales_Report_{$branch_id}_") !== false;
});
...
if (empty($files)) {
    echo "<p class='text-center text-muted'>No recent Party Wise Sales reports found.</p>";
} else {
    ...
    foreach ($files as $file) {
        if (is_file($directory . $file)) {
            ...
            if (preg_match("/Party_wise_Sales_Report_{$branch_id}_(\d{2}-\d{2}-\d{4})_to_(\d{2}-\d{2}-\d{4})/", $file, $matches)) {
            ...
            <div class='report-item d-flex align-items-center p-2 border rounded shadow-sm'>
                ...
                <div class='report-details flex-grow-1'>
                    <p class='report-title mb-0'><strong>Party Wise Sales Report</strong></p>
