...
<!-- [ Main Content ] start -->
        ...
        <?php
          function getinvoiceDetails($conn, $inv_id) {
            ...
            $query = "SELECT q.*, c.*, a.*, qi.*, 
                     im.in_ex_gst, im.net_price, im.gst_rate, im.price as imprice, 
                     im.hsn_code AS inventory_hsn, im.units AS inventory_units,im.opening_stock,im.balance_stock
              FROM quotation q
  ...
  $hsn_code = empty($row['hsn_code']) ? $row['inventory_hsn'] : $row['hsn_code'];
            $units_val = empty($row['units']) ? $row['inventory_units'] : $row['units'];
              $is_out_of_stock = ($row['balance_stock'] < $row['qty']) ? " (Out of Stock)" : "";
                   ...
                   $invoiceItems[] = [
                    ...
                    'units_val' => $units_val, 
                    'hsn_code' => $hsn_code,
                    'stock_status' => $is_out_of_stock, // Display stock status
                    'opening_stock' => $row['opening_stock'], // Store available stock
                    'balanace_stock' => $row['balance_stock']
          ...
          <div class="table-responsive">
            <div class="row">
              <div class="col-sm-12">
                <div class="">
                  <div class="card-body">
                    <form action="convert-quotation-into-invoice-db.php" method="POST" enctype="multipart/form-data">
...
<input type="text" name="business_state" id="business_state" value="<?php echo htmlspecialchars($user['state']); ?>" hidden>
  ...
  $invoice_code = "INV0".(1);
                ...
                <label class="form-control col-sm-5" for="invoice_code">Invoice No</label>
                          ...
                          <div class="py-1 input-group">
                            <input class="form-control" type="date" id="invoice_date" name="invoice_date" value="" required/>
                            <label class="form-control col-sm-5" for="invoice_date">Purchase Date</label>
                          </div>
                          <div class="py-1 input-group">
                            <input class="form-control" type="date" id="dueDate" name="dueDate" value="" required>
    ...
    document.getElementById('dueDate').value = formattedDueDate;
                      ...
                      <div class="row" id="customer_data">
                        <div class="col-md-4 border-left border-bottom border-dark p-3">
                          <div>
                            <h6>Customer Info</h6>
                            <span><?php echo $invoiceDetails['customer_name'];?></span><br/>
    ...
    if ($result->num_rows > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            // Normalize and check if it's a service (fixing the issue)
            $catlogType = strtolower(trim($row["catlog_type"])); 
            $isService = ($catlogType === "services"); // Corrected "services" to "service"

            // Check stock only for products
            $isOutOfStock = (!$isService && isset($row["balance_stock"]) && intval($row["balance_stock"]) <= 0);
            $stockStatus = $isOutOfStock ? " (Out of Stock)" : "";

            ?>
            <option value="<?php echo htmlspecialchars($row["name"]); ?>"  
                    data-productid="<?php echo $row["id"]; ?>" 
                    data-stock="<?php echo htmlspecialchars($row["balance_stock"]); ?>" data-catlogType="<?php echo htmlspecialchars($row["catlog_type"]); ?>">
                <?php echo htmlspecialchars($row["name"]) . (!$isService && $isOutOfStock ? $stockStatus : ""); ?>  
            </option>
            <?php
        }
    } else {
              ...
              <!-- <div class="col-md-2 p-1 border-right border-bottom" id="pricevalbox"> -->
               ...
               <input type="text" class="form-control" name="cess_amount" id="cess_amount" value="" hidden>
                ...
                <input type="text" name="in_ex_gst" id="in_ex_gst" value="" hidden>
                 <input type="text" name="hsn_code" id="hsn_code" value="" hidden>
                  <input type="text" name="units" id="units" value="" hidden>
              <!-- </div> -->
                            ...
                            <tr>
                              <td>
                                <?php echo $item['product']; ?>
                                 ...
                                 <span style="color: red;"><?php echo $item['stock_status']; ?></span>
                              </td>
                              ...
                              <td data-field="pqty"> <input type="number" class="form-control" name="products[<?php echo $index; ?>][pqty]" value="<?php echo $item['qty']; ?>" >
<input type="number" class="form-control" name="products[<?php echo $index; ?>][stock]" value="<?php echo $item['balanace_stock']; ?>" hidden>
                              </td>

                              <td data-field="price"> <input type="text" class="form-control" name="products[<?php echo $index; ?>][pprice]" value="<?php echo $item['price']; ?>"></td>
...
<!-- <td data-field="cess_amount"><span><?php echo $item['cess_amount']; ?></span></td> -->
                              ...
                              <input name="products[<?php echo $index; ?>][phsn_code]" value="<?php echo $item['hsn_code']?>" hidden/>

            <input name="products[<?php echo $index; ?>][punits_val]" value="<?php echo $item['units_val']?>" hidden/>
                ...
                <?php
             $invoice_id = $_GET['id']; // Get invoice ID from URL or form

// Fetch additional charges
$additionalChargesQuery = "SELECT charge_type, charge_price FROM invoice_additional_charges WHERE invoice_id = ?";
...
?>
    ...
    <div class="additional-charges-list">
        <?php foreach ($existingCharges as $index => $charge) { ?>
            <div id="charge_<?php echo preg_replace('/\s+/', '_', $charge['charge_type']); ?>" class="additional-charge-row">
                <div class="row align-items-center">
                    <div class="col-5 text-right">
                        <span class="charge-name"><?php echo htmlspecialchars($charge['charge_type']); ?></span>
                    </div>
                    <div class="col-2">
                        <button type="button" onclick="removeCharge('charge_<?php echo preg_replace('/\s+/', '_', $charge['charge_type']); ?>')" class="btn btn-link text-danger">Remove</button>
                    </div>
                    <div class="col-5">
                        <input type="number" class="form-control charge-input text-right" value="<?php echo number_format($charge['charge_price'], 2); ?>" 
                        oninput="syncChargeValue(this, 'charge_<?php echo preg_replace('/\s+/', '_', $charge['charge_type']); ?>')" style="width: 100%;">
                    </div>
                </div>
                <input type="hidden" name="additionalCharges[charge_type][]" value="<?php echo htmlspecialchars($charge['charge_type']); ?>">
                <input type="hidden" name="additionalCharges[charge_price][]" value="<?php echo number_format($charge['charge_price'], 2); ?>">
            </div>
        <?php } ?>
    </div>
            ...
            <?php include("transportation-details.php"); ?>
                      ...
                      <div class="row col-md-12 text-center pt-3">
                        <div class="col-md-2"><input type="submit" class="btn btn-primary " name="update" value="Create" /></div>
...
const hsn_code = parseFloat($('#hsn_code').val()) || 0; 
...
console.log(totalAmount);
        ...
        <tr>
         <td>${product}<input type="hidden" name="products[${count}][pname]" value="${product}"><input type="hidden" name="products[${count}][pproductid]" value="${productid}"></td>
          ...
          <input type="hidden" class="form-control" name="products[${count}][phsn_code]" value="${hsn_code}" >
           <input type="hidden" class="form-control" name="products[${count}][punits_val]" value="${units}" >
...
function getAdditionalChargesTotal() {
    ...
    console.log("from additional chages");
    document.querySelectorAll(".additional-charge-row input[name='additionalCharges[charge_price][]']").forEach(charge => {
        additionalChargesTotal += parseFloat(charge.value) || 0;
    });
...
function calculate_totals() {
 ...
 let stock = parseInt(row.find('input[name*="[stock]"]').val()) || 0; // Get available stock from hidden field

        console.log("Quantity changed: ", qty);
        console.log("Available Stock: ", stock);

        if (qty > stock) {
            alert("The quantity entered exceeds available stock!");
            $(this).val(qty); // Reset to available stock
        }
        ...
        let cgst = 0, sgst = 0, igst = 0,rowTotal=0;
        ...
        // Calculate row total
         rowTotal = taxableAmount + gstAmount + cessAmount;
...
let additionalChargesTotal = getAdditionalChargesTotal();
grandTotal += additionalChargesTotal;
console.log("Updated Additional Charges:", additionalChargesTotal);
...
function addCharge() {
    ...
    if (selectedOption.value) {
        ...
        const formattedChargeId = chargeName.replace(/\s+/g, "_"); // Convert charge name to a safe ID

        // Check for duplicate charges
        if (document.getElementById("charge_" + formattedChargeId)) {
        ...
        row.id = "charge_" + formattedChargeId;
        ...
        // Calculate totals after adding charge
...
function removeCharge(chargeId) {
    ...
    if (chargeRow) {
        chargeRow.remove();
        setTimeout(calculate_totals, 0); // Recalculate totals after removal
    }
}

function syncChargeValue(input, chargeId) {
    const hiddenInput = document.querySelector(`#${chargeId} input[name='additionalCharges[charge_price][]']`);
    if (hiddenInput) {
        hiddenInput.value = input.value;
    }
    setTimeout(calculate_totals, 0);  // Trigger recalculation after updating value
}
