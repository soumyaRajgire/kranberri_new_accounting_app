...
$itemno++;
    ...
    $productids = isset($product['pproductid']) ? mysqli_real_escape_string($conn, $product['pproductid']) : '';
    $in_ex_gst_val = isset($product['pin_ex_gst']) ? mysqli_real_escape_string($conn, $product['pin_ex_gst']) : '';
...
$itemno++;
    ...
    $productids = isset($product['pproductid']) ? mysqli_real_escape_string($conn, $product['pproductid']) : '';
    $in_ex_gst_val = isset($product['pin_ex_gst']) ? mysqli_real_escape_string($conn, $product['pin_ex_gst']) : '';
    $hsn_code = isset($product['phsn_code']) ? mysqli_real_escape_string($conn, $product['phsn_code']) : '';
    $units_val = isset($product['punits_val']) ? mysqli_real_escape_string($conn, $product['punits_val']) : '';
        //     foreach ($_POST["products"] as $key => $product) {
        //         $itemno++;
        //         $qtyvalue = floatval($_POST['pqty'][$key]);
        //         $priceval = floatval($_POST['pprice'][$key]);
        //         $gstval = floatval($_POST['gst'][$key]);
        //         $discountval = floatval($_POST['discount'][$key]);
        //         $cgstval = $_POST['cgst'][$key];
        //         $sgstval = $_POST['sgst'][$key];
        //         $igstval = $_POST['igst'][$key];
        //         $cessamountval = $_POST['cess_amount'][$key];
        //         $cessrateval = $_POST['cess_rate'][$key];
        //         $proddesc = $_POST['pdesc'][$key];
        //        $productids = $_POST['pproductid'][$key];
        // $totalval = $_POST['ptotal'][$key];
        // $in_ex_gst_val = $_POST['pin_ex_gst'][$key];
        //  $hsn_code = $_POST['hsn_code'][$key];
        //   $units_val = $_POST['units_val'][$key];
        //   $products_name = $_POST['pname'][$key];
        ...
        $l1 = $line_tot + $cgstval + $sgstval + $igstval;
          ...
          $pdf->Cell($widths[2], 8, number_format($priceval, 2), 1, 0, 'C');
                ...
                $pdf->Cell($widths[11], 8, number_format($l1, 2), 1, 1, 'C'); // Total column
                 ...
                 $sql = "INSERT INTO `invoice_items` (`invoice_id`, `itemno`,`productid`, `product`, `prod_desc`, `qty`, `price`, `discount`,`line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`,`hsn_code`,`units`)  VALUES ('$id', '$itemno', '$productids','$products_name', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$l1', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst_val','$hsn_code','$units_val')";
...
$pdf->InvoiceBody($invoice_code, $transportMode, $purchaseDate, $vehicleNo, $customer_name);
    ...
    $pdf2doc4 = $pdf->Output('S');
    
    
$write_result = file_put_contents($filename4, $pdf2doc4);
if ($write_result === false) {
    echo "<script>alert('Error writing file for Invoice TEMPLATE 2.');</script>";
} else {
    //echo "<script>alert('Invoice TEMPLATE 2 created successfully at ".$filename2."');</script>";
 ...
 // Insert main invoice
       $sql = "INSERT INTO `invoice` (`id`, `invoice_code`, `customer_id`, `customer_name`, `customer_email`, `invoice_date`, `due_date`, `total_amount`, `total_gst`, `total_cess`, `grand_total`,`due_amount`, `terms_condition`, `note`, `status`, `branch_id`, `created_by`,`invoice_file_template1`,`invoice_file_template2`,`invoice_file_template3`,`invoice_file_template4` ) 
                VALUES ('$id', '$invoice_code', '$cst_mstr_id', '$customer_name', '$customer_email', '$purchaseDate', '$dueDate', '$final_taxable_amt', '$final_gst_amount', '$final_cess_amount', '$total_amount', '$total_amount','$terms', '$note', 'pending', '$branch_id', '$created_by','$filename1','$filename2','$filename3','$filename4')";
