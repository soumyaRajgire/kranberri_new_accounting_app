          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <div class="page-header-title">
                <h4 class="m-b-10">Create Bill of Supply</h4>
              </div>
              <ul class="breadcrumb">
    ...
    <div class="table-responsive">
      <div class="row">
        <div class="col-sm-12">
          <div class="">
            <div class="card-body">
                <form action="bill_supply_db.php" method="POST" enctype="multipart/form-data">
...
<input type="text" name="business_state" id="business_state" value="<?php echo htmlspecialchars($user['state']); ?>" hidden>
                      ...
                      <div class="col-md-4 pt-1">
                          <div class="py-1 input-group">
                            <?php
$result1 = mysqli_query($conn, "SELECT id FROM bill_of_supply WHERE id=(SELECT max(id) FROM bill_of_supply)");
    ...
    $billNo = "BILL0".(1);
                            ...
                            ?>
                <input class="form-control" type="text" id="billNo" value="<?php echo $billNo; ?>" name="billNo" />
  <select class="form-control" name="billType">
      <option value="cal">cal</option>
      <option value="fin">fin</option>
  </select>
                <label class="form-control col-sm-5" for="billNo">Bill No</label>
                  
                          </div>
                          <div class="py-1 input-group">
                              <input class="form-control" type="date" id="billDate" name="billDate" required/>
                              <label class="form-control col-sm-5" for="billDate">Bill Date</label>
                          </div>
                          <div class="py-1 input-group">
        ...
        <button type="button" class="btn btn-primary btn-sm float-right" data-toggle="modal" data-target="#addCustomersModal" style="margin-top: -10px; height: 25px; font-size: 12px;"><i class="fa fa-plus"></i> <b>New</b></button>
                    <h6>Customer info</h6>
                      <div class="form-group" >
                            <input class="form-control" list="customer_name" name="customer_name_choice" id="customer_name_choice" onchange="checknamevalue(this.value)" autocomplete="off" required />
...
<script>
  function calc_total() {
  let subTotal = 0;
  let packPrice = parseFloat(document.getElementById("pack_price").value) || 0;
  
  // Loop through each row to calculate the subtotal
  document.querySelectorAll("#item-list tbody tr").forEach(function(row) {
  ...
  // Update the sub-total and grand total
  document.getElementById("sub_total").textContent = subTotal.toFixed(2);
  document.getElementById("gtotal").textContent = (subTotal + packPrice).toFixed(2);

  // Update hidden input fields
  document.querySelector("[name='sub_total']").value = subTotal.toFixed(2);
  document.querySelector("[name='total_amount']").value = (subTotal + packPrice).toFixed(2);
}
