...
$invoice_id = safe_get($_POST, 'invoice_id');
...
$pinvoice_code = safe_get($_POST, 'pinvoice_code');
$dueDate = safe_get($_POST, 'due_date');
$pinvoice_date = safe_get($_POST, 'pinvoice_date');
...
foreach ($products as $key => $val) {
    ...
    if($row2 = mysqli_fetch_array($result2)) {
        ...
        $id1 = 1; // In case there are no existing records
    }
    $line_tot = $qty * $price;
    $gstamt = number_format(($gst / 2) * $total_val / 100, 2, '.', '');

    mysqli_query($conn,"INSERT INTO `purchase_invoice_items` (`id`, `itemno`, `pinvoice_id`, `product_id`, `product`, `prod_desc`, `qty`, `price`, `line_total`, `gst`, `gst_amt`, `total`, `created_by`) VALUES ('$id1', '$itemnum_val', '$invoice_id', '$productid', '$product','$description','$qty','$price', '$line_tot', '$gst', '$gstamt', '$tot', '$created_by')");
}
...
$sql = "UPDATE purchase_invoice SET pinvoice_file='$filename' WHERE id='$invoice_id'";
