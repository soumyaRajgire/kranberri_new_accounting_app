...
<!-- [ Main Content ] start -->
        ...
        <?php
          function getinvoiceDetails($conn, $inv_id) {
            ...
            // $query = "SELECT q.*, c.*, a.*, qi.*, im.in_ex_gst, im.net_price, im.gst_rate, im.price, i.invoice_code 
            //           FROM debit_note q
            //           JOIN customer_master c ON q.customer_id = c.id
            //           JOIN pi_invoice i ON i.id = q.purchase_invoice_id
            ...
            (SELECT ii.qty FROM pi_invoice_items ii WHERE ii.invoice_id = q.purchase_invoice_id AND ii.product = qi.product LIMIT 1) AS original_qty,
            (SELECT ii.price FROM pi_invoice_items ii WHERE ii.invoice_id = q.purchase_invoice_id AND ii.product = qi.product LIMIT 1) AS original_price,
               (SELECT SUM(ci.qty) FROM debit_note_items ci WHERE ci.productid = qi.productid AND ci.dnote_id != q.id) AS credited_qty
     FROM debit_note q
     JOIN customer_master c ON q.customer_id = c.id
     JOIN pi_invoice i ON i.id = q.purchase_invoice_id
     JOIN address_master a ON c.id = a.customer_master_id
        ...
        <div class="row align-items-center">
          <div class="col-md-12">
            <div class="page-header-title">
              <h4 class="m-b-10">Edit Debit Note</h4>
            </div>
            <ul class="breadcrumb">
          ...
          <div class="table-responsive">
            <div class="row">
              <div class="col-sm-12">
                <div class="">
                  <div class="card-body">
                    <form action="edit_debitnotedb.php" method="POST" enctype="multipart/form-data">
...
<input type="text" name="business_state" id="business_state" value="<?php echo htmlspecialchars($user['state']); ?>" hidden>
                        ...
                        <div class="col-md-4 pt-1">
                          <div class="py-1 input-group">
                            <input class="form-control" type="text" id="purchaseNo" value="<?php echo $invoiceDetails['dnote_code'] ?>" name="purchaseNo" />
                            <label class="form-control col-sm-5" for="purchaseNo">Debit Note No</label>
                          </div>
                          <div class="py-1 input-group">
                            <input class="form-control" type="date" id="purchaseDate" name="purchaseDate" value="<?php echo $invoiceDetails['dnote_date']?>" required/>
      ...
      <div class="row border-dark border-right border-left border-top border-bottom" id="box_loop_1">
          ...
          <div class="col-md-3 p-1 border-right border-left border-bottom">
              ...
              <!-- </div> -->
                      ...
                      <div class="row border border-dark table-responsive">
   ...
   <tr>
    ...
    <input type="number" class="form-control price-input1"  name="products[<?php echo $index; ?>][pprice]" value="<?php echo isset($item['price']) ? $item['price'] : 0; ?>"    min="1"  
        ...
        <td>
            <!-- Discount -->
            <input type="text" class="form-control" name="products[<?php echo $index; ?>][discount]" value="<?php echo $item['discount']; ?>">
        </td>
        <td>
            <!-- GST -->
            <input type="text" class="form-control" name="products[<?php echo $index; ?>][gst]" value="<?php echo $item['gst']; ?>">
        ...
        <td>
            <!-- IGST -->
            <input type="text" class="form-control" name="products[<?php echo $index; ?>][igst]" value="<?php echo $item['igst']; ?>">
        </td>
        <td data-field="cess_rate">
            <span><?php echo number_format((float)$item['cess_rate'], 2, '.', ''); ?></span>
            <input type="hidden" name="products[<?php echo $index; ?>][cess_rate]" value="<?php echo number_format((float)$item['cess_rate'], 2, '.', ''); ?>">
