...
echo $v = "SELECT *  FROM customer_master JOIN address_master ON customer_master.id = address_master.customer_master_id";
if ($row1 = mysqli_fetch_array($result1)) {
...
$header = ['#', 'Product Description', 'RATE', 'QTY', 'DIS(%)', 'Taxable Amt','GST(%)','CGST', 'SGST', 'IGST', 'CESS', 'TOTAL'];
        ...
        $pdf->Cell($widths[11], 8, number_format($line_tot, 2), 1, 1, 'C'); // Total column
         ...
         $sql = "INSERT INTO `invoice_items` (`invoice_id`, `itemno`,`productid`, `product`, `prod_desc`, `qty`, `price`, `discount`,`line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`,`hsn_code`,`units`) 
                        VALUES ('$id', '$key', '$productids','$product', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$line_tot', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst_val','$hsn_code','$units_val')";

                if (!$conn->query($sql)) {
                    throw new Exception("Failed to insert invoice items: " . $conn->error);
                }

    //           echo $sql=" UPDATE inventory_master SET sold_stock = sold_stock + {$qtyvalue},
...
// The SQL query with placeholders
 ...
 echo $sql1 = "UPDATE inventory_master 
        SET opening_stock = opening_stock, sold_stock = sold_stock + ?, 
            last_updated_by = ?, 
            last_updated_at = NOW() 
        WHERE id = ?";
