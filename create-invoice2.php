...
$query = "SELECT temp_name FROM invoice_temp WHERE status = 'active' LIMIT 1";
$result = $conn->query($query);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $activeTemplate = 'save_'.$row['temp_name'].'.php'; 
     //echo "<script> alert('Selecyed Template is \"$activeTemplate\" !'); </script>";
} else {
    $activeTemplate = 'save_invoice.php';
     echo "<script> alert('Error in Selecting template: " . $conn->error . "'); </script>";
}
...
//                     oninput="calculate_totals()" style="width: 100%;">
//                 </div>
//             </div>
//         `;
