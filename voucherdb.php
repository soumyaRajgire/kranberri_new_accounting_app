...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $res1 = mysqli_query($conn, "SELECT SUM(paid_amount) AS paid_amount FROM voucher WHERE voucher_id = '$invoice_pid'");
...
if ($row1 = mysqli_fetch_array($result1)) {
    ...
    $pdf->SetFont('Arial', '', 9);
    $pdf->Cell(140, 6, "Phone: {$row1['phone']}", 'L', 0, 'L');
    $pdf->Cell(0, 6, "Date : $receipt_date", 'R', 1, 'L');
    $pdf->Cell(140, 6, "Email: {$row1['email']}", 'L', 0, 'L');
    $pdf->Cell(0, 6, "Created By : $created_by", 'R', 1, 'L');
    $pdf->Cell(140, 6, "GST : {$row1['gstin']}", 'LB', 0, 'L');
    $pdf->Cell(0, 6, "", 'RB', 1, 'L');
}
    $pdf->SetFont('Arial', '', 9);
    $table = array(
...
file_put_contents($filename, $pdfdoc);
    ...
    $sql = "INSERT INTO voucher (
        ...
        '$receipt_no', '$receipt_date', '$customer_id', '$invoice_pid', 
...
if (mysqli_query($conn, $sql)) {
    ...
    '$customer_name', 
    '$amount', 
    'D',  -- Ensuring only debit is recorded
    '$receipt_no', 
    '$branch_id'
)";
...
if (!$conn->query($sql_ledger)) {
    throw new Exception("Failed to save ledger details: " . $conn->error);
}



        $sql_reconcile = "INSERT INTO voucher_reconciliation ( `voucher_id`, `pi_invoice_id`, `reconciled_amount`, `reconciliation_date`
        ) VALUES ( '$receipt_id', '$invoice_pid', '$amount', NOW())";

        if (mysqli_query($conn, $sql_reconcile)) {

$sql2 = "UPDATE pi_invoice SET  due_amount='$due_amount',status = '$status' WHERE id = '$invoice_pid'";
  if (mysqli_query($conn, $sql2)) {
            ...
            echo "<script type='text/javascript'>
                    ...
                    window.location.href = 'manage-voucher.php';
                  </script>";
        }
    } else {
        echo "<script type='text/javascript'>
