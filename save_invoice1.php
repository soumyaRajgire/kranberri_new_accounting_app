            ...
            foreach ($_POST["products"] as $key => $product) {
               ...
               $productids = $_POST['productids'][$key];
                ...
                $pdf->Cell($widths[11], 8, number_format($line_tot, 2), 1, 1, 'C'); // Total column
                 ...
                 $sql = "INSERT INTO `invoice_items` (`invoice_id`, `itemno`,`productid`, `product`, `prod_desc`, `qty`, `price`, `discount`,`line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`) 
                                VALUES ('$id', '$key', '$productids','$product', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$line_tot', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst_val')";
        
                        if (!$conn->query($sql)) {
                            throw new Exception("Failed to insert invoice items: " . $conn->error);
                        }
            //            $sql=" UPDATE inventory_master SET opening_stock = opening_stock - {$qtyvalue},
            // sold_stock = sold_stock + {$qtyvalue},
            // last_updated_by = '{$created_by}',
            // last_updated_at = NOW() where id= {$productids}";
...
$pdfType = "Invoice";
$destinationURL="view-invoices.php";
