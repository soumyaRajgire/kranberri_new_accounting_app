...
$cnote_code = mysqli_real_escape_string($conn, $_POST['creditNoteNo'] ?? '');
$cnoteDate = mysqli_real_escape_string($conn, $_POST['creditNoteDate'] ?? date('Y-m-d'));
...
if ($customer_name != "") {
   ...
   $pdf->plot_table($widths, $lineheight, $table,$border,$aligns);
    ...
    // Title
    $table = array(array("GST Credit Note"));
    ...
    if ($row1 = mysqli_fetch_array($result1)) {
        ...
        $pdf->plot_table(array(47.25, 47.25, 47.25, 47.25), 9, array(
            array("Credit Note Number", "$cnote_code", "Place of Supply", "{$row1['s_state']}"),
            array("Credit Note Date", "$cnoteDate", "Created By", "$created_by"),
            array("Credit Note Type", "Original")
        ), 1, array('L', 'L', 'L', 'L'));
    }
    ...
    try {
        // Insert `credit_note`
        $sql = "INSERT INTO credit_note (id, cnote_code, cnote_file, customer_id, customer_name, email, cnote_date, total_amount, terms_condition, note, status, created_by, branch_id) 
        VALUES ('$id', '$cnote_code', '$filename', '$cst_mstr_id', '$customer_name', '$customer_email', '$cnoteDate', '$total_amount', '$terms', '$note', 'pending', '$created_by', '$branch_id')";
        ...
        foreach ($_POST["products"] as $key => $val) {
            ...
            $conn->query("INSERT INTO credit_note_items (`cnote_id`, `product_name`, `prod_desc`, `qty`, `price`, `line_total`, `gst`, `gst_amt`, `total`, `created_by`) 
        ...
        $pdf->Cell(30, 6, "Credit Note Total", 'B', 0, 'R');
        ...
        alert('Successfully Created Credit Note');
    </script>";
    } catch (Exception $e) {
        $conn->rollback();
        echo "<script>alert('Error creating credit note: " . addslashes($e->getMessage()) . "');</script>";
    }
