...
$dnote_id = mysqli_real_escape_string($conn, $_POST['inv_id']);
...
$dnote_code = mysqli_real_escape_string($conn, $_POST['purchaseNo'] ?? '');
$dnote_date = mysqli_real_escape_string($conn, $_POST['purchaseDate'] ?? date('Y-m-d'));
...
if (!empty($delete_item_ids)) {
    ...
    // Fetch the quantities of the deleted products before deletion
    $sql_get_deleted_items = "SELECT productid, qty FROM debit_note_items WHERE id IN ($delete_item_ids)";
    $result_deleted_items = $conn->query($sql_get_deleted_items);

    while ($row = $result_deleted_items->fetch_assoc()) {
        $productid = $row['productid'];
        $deleted_qty = $row['qty'];

        // Update the stock by adding back the deleted quantity
        $sql_update_stock = "UPDATE inventory_master 
                             SET stock_out = stock_out - $deleted_qty,
                                 balance_stock = (opening_stock + stock_in) - stock_out
                             WHERE id = $productid";

        if (!$conn->query($sql_update_stock)) {
            throw new Exception("Error updating stock for deleted product: " . $conn->error);
        }

        // Log this stock change in stock_master (for auditing purposes)
        // $sql_stock_entry = "INSERT INTO stock_master (product_id, reference_no, quantity, add_and_deduct, remark, date, created_by, created_on) 
        //                     VALUES (?, ?, ?, ?, ?, NOW(), ?, NOW())";

        // $stmt_stock_entry = $conn->prepare($sql_stock_entry);
        // $transaction_type = "Debit Note Deletion"; 
        // $remark = "Debit Note Item Deleted";
        // $stmt_stock_entry->bind_param("isisss", $productid, $dnote_code, $deleted_qty, $transaction_type, $remark, $created_by);
        // $stmt_stock_entry->execute();
        // $stmt_stock_entry->close();
    }

    // Now delete the products from the debit_note_items table
    $conn->query("DELETE FROM debit_note_items WHERE id IN ($delete_item_ids)");
}
...
$result2 = mysqli_query($conn, "SELECT *  FROM customer_master JOIN address_master ON customer_master.id = address_master.customer_master_id WHERE contact_type='Supplier'");
...
function convertNumberToWords($number) {
    ...
    $sql_old_quantity = "SELECT qty FROM debit_note_items WHERE dnote_id = ? AND productid = ?";
    $stmt_old = $conn->prepare($sql_old_quantity);
    $stmt_old->bind_param("ii", $dnote_id, $productid);
    $stmt_old->execute();
    $result_old = $stmt_old->get_result();
    $row_old = $result_old->fetch_assoc();
    $old_quantity = $row_old['qty'] ?? 0;
    $stmt_old->close();
    $sql_update_item = "UPDATE debit_note_items 
        SET productid = '$productid',
            ...
            sgst='$sgst',
            igst='$igst'
        WHERE dnote_id = '$dnote_id' AND productid = '$productid'";

    if (!$conn->query($sql_update_item)) {
        throw new Exception("Error updating debit note item: " . $conn->error);
    }
...
$remark = "Debit Note Updated";

$stmt_stock_entry->bind_param("isisss", $productid, $dnote_code, $quantity, $transaction_type, $remark, $created_by);
$stmt_stock_entry->execute();
$stmt_stock_entry->close();

        // Update stock
        $stock_difference = $old_quantity - $quantity;
        $sql_update_stock = "UPDATE inventory_master 
        ...
        $stmt_update->bind_param("ii", $stock_difference, $productid);
        $stmt_update->execute();
        $stmt_update->close();
...
// encode data (puts attachment in proper format)
 ...
 file_put_contents($filename, $pdfdoc);
   ...
   $sql_update_debit_note = "UPDATE debit_note  SET dnote_date = '$dnote_date', total_amount = '$total_amount1', dnote_file = '$filename', adjusted_amount = '$adjusted_amount', terms_condition = '$terms', note = '$note', total_gst_amount='$final_gst_amount', total_cess_amount='$final_cess_amount',
            created_by = '$created_by',created_at = NOW() WHERE id = '$dnote_id'";

    if (!$conn->query($sql_update_debit_note)) {
        throw new Exception("Error Updating Debit Note: " . $conn->error);
    }


    // Update Ledger
    $sql_update_ledger = "UPDATE ledger 
    SET transaction_date = '$dnote_date',
        amount = '$total_amount1',
        branch_id = '$branch_id'
    WHERE voucher_id = '$dnote_id' AND transaction_type = 'Debit Note'";
    ...
    // Commit Transaction
    $conn->commit();
    echo "<script>alert('Debit Note updated successfully!'); window.location.href = 'manage-debitnote.php';</script>";
//    echo "<script>
//     Swal.fire({
//         icon: 'success',
//         title: 'Success',
