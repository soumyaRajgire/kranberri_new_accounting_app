...
<?php
// Assuming you already have a valid database connection stored in $conn

// SQL query to fetch inventory data
$query = "SELECT `id`, `inventory_type`, `can_be_sold`, `catlog_type`, `name`, `category`, `company_name`, `price`, `in_ex_gst`, `gst_rate`, `non_taxable`, `net_price`, `hsn_code`, `SAC_Code`, `units`, `cess_rate`, `cess_amt`, `sku`, `opening_stock`, `opening_stockdate`, `min_stockalert`, `max_stockalert`, `Stock_in`, `stock_out`, `balance_stock`, `description`, `remark`, `created_by`, `created_on`, `last_updated_by`, `last_updated_at` FROM `inventory_master` WHERE 1";
...
if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    ...
    // Calculate the value of price * balance_stock
    $calculated_value = $row['price'] * $row['balance_stock'];
    
    // Safely echo the calculated value (though it's not strictly necessary for numbers)
    echo '<td>' . htmlspecialchars($calculated_value) . '</td>';

    echo '<td>' . htmlspecialchars($row['price']) . '</td>';
    echo '<td>' . htmlspecialchars($row['net_price']) . '</td>';
    echo '<td>' . htmlspecialchars($row['balance_stock']) . ' ' . htmlspecialchars($row['units']) . '</td>';
    echo '</tr>';
}
