...
$files = array_filter(scandir($directory, SCANDIR_SORT_DESCENDING), function ($file)  use ($branch_id)  {
    return strpos($file, "Individual_Party_Wise_Sales_Report_Branch_{$branch_id}_") !== false; // Only fetch relevant reports
});
...
if (empty($files)) {
    echo "<p class='text-center text-muted'>No recent individual party-wise sales reports found.</p>";
} else {
    ...
    foreach ($files as $file) {
        if (is_file($directory . $file)) {
            ...
            // Extract date range from the filename and reformat to DD/MM/YYYY
            if (preg_match("/Individual_Party_Wise_Sales_Report_Branch_{$branch_id}_(\d{2})-(\d{2})-(\d{4})_to_(\d{2})-(\d{2})-(\d{4})_(\d{14})/", $file, $matches)) {
            ...
            <div class='report-item d-flex align-items-center p-2 border rounded shadow-sm'>
                ...
                <div class='report-details flex-grow-1'>
                    <p class='report-title mb-0'><strong>Individual Party Sales Report</strong></p>
