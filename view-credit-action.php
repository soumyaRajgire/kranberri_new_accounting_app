...
<body class="">
    ...
    <!-- [ Pre-loader ] start -->
        <?php
        function getcreditNoteDetails($conn, $inv_id) {
...
$bid  = $_SESSION['branch_id'];
    ...
    $query = "SELECT q.*, c.email AS customerEmail, c.*, a.*, qi.*
              FROM credit_note q
...
$inv_id = $_GET['inv_id'];
$creditNoteDetails = getcreditNoteDetails($conn, $inv_id);
...
if (isset($_GET['action']) && $_GET['action'] === 'delete_permanent' && isset($_GET['inv_id'])) {
    ...
    try {
        ...
        $query = "SELECT productid, qty FROM credit_note_items WHERE cnote_id = '$inv_id'";
        $result = $conn->query($query);
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                // **Reverse Inventory Update (Decrease stock_in)**
                // $updateInventory = "UPDATE inventory_master 
                //                     SET stock_in = GREATEST(stock_in - $quantity, 0), 
                //                         balance_stock = (opening_stock + GREATEST(stock_in - $quantity, 0)) - stock_out
                //                     WHERE id = '$product_id'";
        ...
        // Remove Ledger Entry
        $deleteLedgerQuery = "DELETE FROM ledger WHERE voucher_id = '$inv_id' AND transaction_type = 'Credit Note'";
        if (!$conn->query($deleteLedgerQuery)) {
            throw new Exception("Error deleting from ledger: " . $conn->error);
        }

        // Fetch PDF File Path Before Deleting Credit Note
        $query = "SELECT cnote_file FROM credit_note WHERE id = '$inv_id'";
        $result = $conn->query($query);
        $pdfFilePath = ($result && $row = $result->fetch_assoc()) ? $row['cnote_file'] : "";

        // Delete Related Items
        $deleteItemsQuery = "DELETE FROM credit_note_items WHERE cnote_id = '$inv_id'";
        if (!$conn->query($deleteItemsQuery)) {
            throw new Exception("Error deleting credit note items: " . $conn->error);
        }
        ...
        echo "<script>alert('Credit Note permanently deleted.'); window.location.href='manage-creditnote.php';</script>";
    } catch (Exception $e) {
        $conn->rollback();
        error_log("Delete Permanent Error: " . $e->getMessage());
        echo "<script>alert('Error deleting Credit Note: " . $e->getMessage() . "'); window.location.href='manage-creditnote.php';</script>";
    }
}
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['inv_id'])) {
    ...
    try {
        ...
        if (!$conn->query($query)) {
            throw new Exception("Error marking credit note as deleted: " . $conn->error);
        }

        // Commit Transaction
        $conn->commit();

        echo "<script>alert('Credit Note deleted successfully.'); window.location.href='manage-creditnote.php';</script>";
    } catch (Exception $e) {
        $conn->rollback();
        ...
        <div class="col-md-6 col-lg-6">
            <ul class="nav">
                <li class="nav-item mt-2">
                    <h5><a href="#" class="customer_est_name text-primary"  style="font-size: 19px;">Customer: <?php echo $creditNoteDetails['customerName'];?></a></h5>
                </li>
            </ul>
        </div>
...
<div class="btn-group">
    ...
    <!-- <a href="<?php echo $porderDetails['invoice_file']; ?>" id="pdfLink" style="display: none;">Download PDF</a> -->
</div>
</div>
...
$query = "SELECT UNIX_TIMESTAMP(created_at) as created_date FROM credit_note WHERE id = ?";
...
?>
 ...
 <!--   <a class="dropdown-item text-danger" href="view-credit-action.php?action=delete_permanent&inv_id=<?php echo $inv_id; ?>" 
    ...
    <a href="" class="btn border border-grey" onclick="printPDF('<?php echo $creditNoteDetails['cnote_file']?>')" data-toggle="tooltip" data-placement="top" title="Print"><i class="fa fa-print text-grey" aria-hidden="true"></i> </a>
    <a href="<?php echo $creditNoteDetails['cnote_file']?>" target="_blank" class="btn border border-grey" data-toggle="tooltip" data-placement="top" title="Download"><i class="fa fa-download text-grey" aria-hidden="true"></i></a>
...
<!-- <iframe id="pdfViewer" src="<?php echo $creditNoteDetails['cnote_file']; ?>" width="100%" height="600px"></iframe> -->
...
<div id="shareModal" class="modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); justify-content: center; align-items: center;">
    <div class="modal-content" style="background: white; padding: 20px; border-radius: 8px; width: 400px; max-width: 90%;">
        ...
        <h3>Compose Email</h3>
        <form id="emailForm" action="send_creditNote_email.php" method="POST" enctype="multipart/form-data">
            <!-- Hidden Inputs -->
            <input type="hidden" name="customer_email" id="customer_email" value="<?php echo isset($creditNoteDetails['customerEmail']) ? htmlspecialchars($creditNoteDetails['customerEmail'], ENT_QUOTES, 'UTF-8') : ''; ?>" />
            <input type="hidden" name="cnote_file" id="cnote_file" value="<?php echo isset($creditNoteDetails['cnote_file']) ? htmlspecialchars($creditNoteDetails['cnote_file'], ENT_QUOTES, 'UTF-8') : ''; ?>" />

            <!-- Visible Inputs -->
            <label for="email">To:</label>
            <input type="email" class="form-control" id="email" name="email" value="<?php echo isset($creditNoteDetails['customerEmail']) ? $creditNoteDetails['customerEmail'] : ''; ?>" required />

            <label for="subject">Subject:</label>
            <input type="text" class="form-control" id="subject" name="subject" value="Credit Note #<?php echo htmlspecialchars($inv_id, ENT_QUOTES, 'UTF-8'); ?>" required />

            <label for="message">Message:</label>
            <textarea class="form-control" id="message" name="message" rows="4" required>
            Dear Customer, please find the attached Credit Note
            </textarea>

            <input type="hidden" name="invoiceId" id="invoiceId" value="<?php echo htmlspecialchars($_GET['porder_id'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" />
...
<script>
 ...
 document.getElementById("emailForm").addEventListener("submit", function(event) {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const invoiceId = new URLSearchParams(window.location.search).get("inv_id");

        fetch(`fetch_creidtNote_path.php?invoice_id=${invoiceId}`)
            ...
            .then(data => {
                if (data.status === "success") {
                    ...
                    // alert("Failed to fetch purchase invoice file: " + data.message);
                }
            })
            .catch(error => {
                console.error("Error fetching purchase invoice file:", error);
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const url = "<?php echo $creditNoteDetails['cnote_file']; ?>"; // Make sure it is echoed as a string
