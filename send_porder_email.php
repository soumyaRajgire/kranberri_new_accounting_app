...
if (isset($_POST['sendMail'])) {
    ...
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        echo '<script>alert("Invalid or missing email address. Please enter a valid email address."); window.location.href = "view-purchase-order.php";</script>';
        exit;
    }
    ...
    try {
        ...
        // Send the email
        if ($mail->send()) {
            echo '<script>alert("Purchase order has been sent successfully!"); window.location.href = "view-purchase-order.php";</script>';
        } else {
