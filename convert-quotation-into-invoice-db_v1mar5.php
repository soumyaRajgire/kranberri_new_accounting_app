...
$l1 = $line_tot + $cgstval + $sgstval + $igstval;
  ...
  $pdf->Cell($widths[2], 8, number_format($priceval, 2), 1, 0, 'C');
        ...
        $pdf->Cell($widths[11], 8, number_format($l1, 2), 1, 1, 'C'); // Total column
         ...
         $sql = "INSERT INTO `invoice_items` (`invoice_id`, `itemno`,`productid`, `product`, `prod_desc`, `qty`, `price`, `discount`,`line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`,`hsn_code`,`units`)  VALUES ('$id', '$key', '$productids','$products_name', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$l1', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst_val','$hsn_code','$units_val')";
