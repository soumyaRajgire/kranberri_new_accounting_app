...
// Insert primary invoice for the session's branch
$sql = "INSERT INTO purchase_invoice (
  id, pinvoice_code, pinvoice_file, customer_id, customer_name, email, 
  pinvoice_date, due_date, total_amount, total_tax, grand_total, 
  terms_condition, note, status, created_by, branch_id
) VALUES (
  '$id', '$pinvoice_code', '$filename', '$cst_mstr_id', '$customer_name', 
  '$customer_email', '$pinvoiceDate', '$dueDate', '$pricevaltot', '$gsttot', 
  '$tot_amt', '$terms', '$note', 'pending', '$created_by', '$branch_id'
)";

// Execute the primary invoice query
if ($conn->query($sql) === TRUE) {
  // Fetch all branches for the business
  $sql_branches = "SELECT branch_id FROM add_branch WHERE business_id = '$business_id'";
  $result_branches = $conn->query($sql_branches);

  if ($result_branches->num_rows > 0) {
      while ($row_branch = $result_branches->fetch_assoc()) {
          $current_branch_id = $row_branch['branch_id'];

          // Skip the branch already saved in the session
          if (isset($branch_id) && $branch_id == $current_branch_id) {
              continue;
          }

          // Generate a new ID for the invoice (unique per branch)
          $result_invoice = mysqli_query($conn, "SELECT id FROM purchase_invoice WHERE id = (SELECT MAX(id) FROM purchase_invoice)");
          if ($row_invoice = mysqli_fetch_array($result_invoice)) {
              $new_id = $row_invoice['id'] + 1;
          } else {
              $new_id = 1;
          }

          // Save the invoice for the current branch
          $sql_branch_invoice = "INSERT INTO purchase_invoice (
              id, pinvoice_code, pinvoice_file, customer_id, customer_name, email, 
              pinvoice_date, due_date, total_amount, total_tax, grand_total, 
              terms_condition, note, status, created_by, branch_id
          ) VALUES (
              '$new_id', '$pinvoice_code', '$filename', '$cst_mstr_id', '$customer_name', 
              '$customer_email', '$pinvoiceDate', '$dueDate', '$pricevaltot', '$gsttot', 
              '$tot_amt', '$terms', '$note', 'pending', '$created_by', '$current_branch_id'
          )";

          // Execute the query for other branches
          if (!$conn->query($sql_branch_invoice)) {
              error_log("Error creating invoice for branch $current_branch_id: " . $conn->error);
          }
      }
  }
  ...
  // Handle errors for the primary invoice insert
  ?>
  <script>
