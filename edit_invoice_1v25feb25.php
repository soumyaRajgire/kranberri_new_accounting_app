...
$query1 = "SELECT DISTINCT q.*, c.*, a.*, qi.*, im.in_ex_gst, im.net_price, im.gst_rate, im.price as imprice
FROM invoice q
LEFT JOIN customer_master c ON q.customer_id = c.id
LEFT JOIN address_master a ON c.id = a.customer_master_id
LEFT JOIN invoice_items qi ON q.id = qi.invoice_id
LEFT JOIN inventory_master im ON qi.productid = im.id
WHERE q.id = '$invoiceId';
    ...
    $invoiceItems = [];
    $no_of_rows=0;
    // Loop through the rest of the rows (invoice items)
    while ($row = $result->fetch_assoc()) {
        $no_of_rows+=1;
        
        $invoiceItems[] = $row;
        
        // echo "<script>alert('Row ".$no_of_rows ." ');</script>";
    }

   
    //echo "Invoice Data:\n";
   // print_r($invoiceData);  // Display the first row (invoice details)
    
    //echo "Invoice Items:\n";
    //print_r($invoiceItems);  // Display the remaining rows (invoice items)
} else {
    echo "No records found.";
}

            $query = "SELECT DISTINCT q.*, c.*, a.*, qi.*, im.in_ex_gst, im.net_price, im.gst_rate, im.price as imprice
FROM invoice q
LEFT JOIN customer_master c ON q.customer_id = c.id
LEFT JOIN address_master a ON c.id = a.customer_master_id
LEFT JOIN invoice_items qi ON q.id = qi.invoice_id
...
//         echo "Number of rows in invoiceItems: " . count($invoiceItems) . "<br>";
// echo "Number of rows in invoiceDetails['invoice_items']: " . count($invoiceDetails['invoice_items']) . "<br>";
...
<input type="text" name="business_state" id="business_state" value="<?php echo htmlspecialchars($user['state']); ?>" hidden>
                        ...
                        <div class="col-md-4 pt-1">
                          <div class="py-1 input-group">
                            <input class="form-control" type="text" id="purchaseNo" value="<?php echo $invoiceDetails['invoice_code'] ?>" name="purchaseNo" readonly />
...
if ($result->num_rows > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $isService = (strtolower($row["catlog_type"]) === "services"); // Check if it’s a service
        $isOutOfStock = (!$isService && $row["opening_stock"] <= 0);  // Only check stock for products
        $stockStatus = $isOutOfStock ? " (Out of Stock)" : "";  // Add "(Out of Stock)" only for products

        ?>
        <option value="<?php echo $row["name"]; ?>"   data-productid="<?php echo $row["id"]; ?>" 
                data-stock="<?php echo $row["opening_stock"]; ?>">  <?php echo $row["name"] . (!$isService ? $stockStatus : ""); ?>  </option>
        <?php
    }
}
                            ?>
                          ...
                          <tbody>
                            <?php
                              ...
                              foreach ($invoiceDetails['invoice_items'] as $item) {
                                echo "<script>alert('Row " . $index . "');</script>";
...
<!-- <td data-field="cess_amount"><span><?php echo $item['cess_amount']; ?></span></td> -->
            ...
            <input name="products[<?php echo $index; ?>][punits_val]" value="<?php echo $item['units_val']?>" hidden/>
                ...
                <?php
             $invoice_id = $_GET['inv_id']; // Get invoice ID from URL or form
...
console.log(totalAmount);
        ...
        <tr>
         <td>${product}<input type="hidden" name="products[${count}][pname]" value="${product}"><input type="hidden" name="products[${count}][pproductid]" value="${productid}"></td>
          ...
          <input type="hidden" class="form-control" name="products[${count}][hsn_code]" value="${hsn_code}" >
           <input type="hidden" class="form-control" name="products[${count}][units_val]" value="${units_val}" >
