...
if (isset($_GET['id'])) {
    ...
    if ($select_stmt = $conn->prepare($select_sql)) {
        ...
        die("Error preparing query: " . $conn->error); // Error handling for query preparation
    }
}
...
if (isset($_POST['submit'])) {
    ...
    $email = $_POST['email'];
    $supplier_gstin = $_POST['supplier_gstin'];
    $supplier_registered_name = $_POST['supplier_registered_name'];
    ...
    $update_supplier_sql = "UPDATE customer_master SET
    title = ?,
    customerName = ?,
    entityType = ?,
    mobile = ?,
    ...
    if ($update_supplier_stmt = $conn->prepare($update_supplier_sql)) {
        $update_supplier_stmt->bind_param("ssssssssssssssssssssssssss", 
            $title, $name, $entity_type, $mobile_number, $email, $supplier_gstin, $supplier_registered_name, $business_name, $additional_business_name, $display_name,
        ...
        $update_supplier_result = $update_supplier_stmt->execute();

        if (!$update_supplier_result) {
            die("Update supplier failed: " . $update_supplier_stmt->error); // Error handling for update
        }

        $update_supplier_stmt->close();
    } else {
        die("Error preparing update statement: " . $conn->error); // Error handling for query preparation
    }
    ...
    if ($update_address_stmt = $conn->prepare($update_address_sql)) {
        ...
        if (!$update_address_result) {
            die("Update address failed: " . $update_address_stmt->error); // Error handling for update
        }

        $update_address_stmt->close();
    } else {
        die("Error preparing address update: " . $conn->error); // Error handling for query preparation
    }

    // If both updates are successful, redirect to suppliers page
if ($update_supplier_result && $update_address_result) {
        ...
        die("Update failed, please check logs.");
    }
}
?>
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h4 class="m-b-10">Update Supplier</h4>
                            </div>
                            <!--  <ul class="breadcrumb">
...
<input type="hidden" name="id" value="<?php echo $row['id']; ?>">
    ...
    <div class="input-group mb-1">
        <div class="input-group-prepend">
            ...
            <div class="did-floating-label-content">
        <input type="text" class="did-floating-input" placeholder="" name="name" id="name"  value="<?php echo isset($row['customerName']) ? $row['customerName'] : ''; ?>">

            <label for="" class="did-floating-label">Name<span class="mandatory-symbol">*</span></label>
        </div>
    </div>
    </div>
    ...
    <div class="mb-1 col-lg-6">
        <div class="did-floating-label-content">
            <input type="text" id="supplier_gstin" name="supplier_gstin" class="did-floating-input" placeholder=""  value="<?php echo $row['gstin']; ?>" >
             <label for="supplier_gstin" class="did-floating-label">Supplier GSTIN</label>
        </div>
    </div>
    <div class="mb-1 col-lg-6">
        <div class="did-floating-label-content">
        <input type="text" id="supplier_registered_name" name="supplier_registered_name" class="did-floating-input" placeholder="" readonly  value="<?php echo $row['gst_reg_name']; ?>" >
    ...
    <div class="row">
        ...
        <div class="mb-1 col-lg-6">
             <div class="did-floating-label-content">
                <select id="bill_state" name="bill_state" class="did-floating-select">
                    <option value="<?php echo $row['b_state']; ?>" selected><?php echo $row['b_state']; ?></option>
        ...
        <div class="mb-1 col-lg-6">
            <div class="did-floating-label-content">
                ...
                <option value="<?php echo $row['b_country']; ?>" selected><?php echo $row['b_country']; ?></option>
      ...
      <div class="text-center">
        ...
        <div class="mb-1 col-lg-6">
            <div class="did-floating-label-content">
                ...
                <option value="<?php echo $row['s_country']; ?>" selected><?php echo $row['s_country']; ?></option>
                    <!-- <option value="">Select a Country</option> -->
                    <option value="Afghanistan">Afghanistan</option>
                    <option value="Albania">Albania</option>
                    <option value="Algeria">Algeria</option>
