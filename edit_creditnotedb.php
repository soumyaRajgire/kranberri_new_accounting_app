...
try {
   ...
   if (!empty($delete_item_ids)) {
        ...
        $sql_get_deleted_items = "SELECT productid, qty FROM credit_note_items WHERE id IN ($delete_item_ids)";
        ...
        while ($row = $result_deleted_items->fetch_assoc()) {
            ...
            $sql_update_stock = "UPDATE inventory_master 
                                 SET stock_in = stock_in - $deleted_qty,
...
$stmt_update->bind_param("ii", $quantity_difference, $productid);
...
$pdf->Cell(70, 6, number_format($final_gst_amount, 2), 1, 1, 'R', true);
...
file_put_contents($filename, $pdfdoc);
   ...
   $sql_update_credit_note = "UPDATE credit_note  SET cnote_date = '$cnote_date',cnote_file='$filename',total_amount = '$total_amount1',adjusted_amount = '$adjusted_amount',terms_condition = '$terms',note = '$note', total_gst_amount='$final_gst_amount',total_cess_amount='$final_cess_amount',
