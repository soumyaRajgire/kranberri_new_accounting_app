        ...
        <?php
          function getinvoiceDetails($conn, $inv_id) {
            ...
            $query = "SELECT q.*, c.*, a.*, qi.*, 
                     ...
                     im.hsn_code AS inventory_hsn, im.units AS inventory_units
              FROM quotation q
  ...
  $invoice_code = "INV0".(1);
    ...
    if ($result->num_rows > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $isOutOfStock = (!$isService && isset($row["opening_stock"]) && intval($row["opening_stock"]) <= 0);
            $stockStatus = $isOutOfStock ? " (Out of Stock)" : "";

            ?>
            <option value="<?php echo htmlspecialchars($row["name"]); ?>"  
                    data-productid="<?php echo $row["id"]; ?>" 
                    data-stock="<?php echo htmlspecialchars($row["opening_stock"]); ?>" data-catlogType="<?php echo htmlspecialchars($row["catlog_type"]); ?>">
