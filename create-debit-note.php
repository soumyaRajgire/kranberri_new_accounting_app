...
<input type="text" name="business_state" id="business_state" value="<?php echo htmlspecialchars($user['state']); ?>" hidden>
    ...
    echo $currentDate;
    $sql = "SELECT i.id, i.invoice_code 
        FROM pi_invoice i
        LEFT JOIN debit_note cn ON i.id = cn.purchase_invoice_id
        WHERE i.branch_id='$bid' 
        AND i.invoice_date >= DATE_SUB('$currentDate', INTERVAL 2 MONTH) GROUP BY i.invoice_code;";
   ...
   // $bid = $_SESSION['branch_id'];
    //$sql = "
      //  SELECT i.id, i.invoice_code 
        //FROM pi_invoice i
        //LEFT JOIN debit_note cn ON i.id = cn.purchase_invoice_id
        //WHERE cn.purchase_invoice_id IS NULL AND i.branch_id='$bid'";
    //$result = mysqli_query($conn, $sql);
    //while ($row = mysqli_fetch_assoc($result)) {
      //  echo "<option value='{$row['id']}'>{$row['invoice_code']}</option>";
    //}
...
<div class="row" id="customer_data"></div>
    ...
    <tr>
            ...
            <th data-field="prod_desc" data-editable="true">Product Desc</th>
             <th data-field="batch_no" data-editable="true" id="batch-header" style="display: none;">Batch No</th>
              ...
              console.log(response);
                try {
                   if (response.status === 'error') {
                    if (response.message === 'No product to credit for this invoice.') {
                        Swal.fire({
                            icon: 'warning',
                            title: 'No Product Available',
                            text: 'There are no products to debit for this purchase invoice.',
...
function populateProductList(items, additionalCharges, invoice) {
     $('#box_loop_1').hide();
var gtotal =0, totgstAmt=0,totcessamt =0,tottax=0;
    // Populate items
    $("#item-list tbody").empty();
       // Check if any items have batch_no to determine visibility
    let hasBatchNo = items.some(item => item.batch_no && item.batch_no !== "");

    // Show or hide the Batch No header based on items' batch numbers
    if (hasBatchNo) {
        $("#batch-header").show();  // Show Batch No column header
    } else {
        $("#batch-header").hide();  // Hide Batch No column header
    }

    items.forEach((item, index) => { // Add `index` as the second argument
       console.log(item.credited_qty);
  const remainingQty = item.qty - item.credited_qty;

  console.log(remainingQty);
var cgst=0, sgst=0, igst=0,gt=0;
 const price = parseFloat(item.price).toFixed(2);
            const totalPrice = (remainingQty * price).toFixed(2);

            // Calculate GST, CGST, SGST, IGST based on remaining quantity
            const gst = parseFloat(item.gst).toFixed(2);
  const lineTotal = (remainingQty * price) - ((price * (item.discount)) / 100);
    const gstAmt = (lineTotal * (gst)) / 100;
    const cessAmt = (lineTotal * (item.cess_rate)) / 100;

tottax = tottax + lineTotal;
gt = lineTotal + gstAmt + cessAmt;
 gtotal = gtotal + gt;
 totgstAmt = totgstAmt + gstAmt;
 totcessamt = totcessamt + cessAmt;

            if(item.credited_qty == 0)
            {
              console.log("from credited qty is 0");
            ...
            {
              console.log("from credited qty is not s0");
               cgst = parseFloat(item.ccgst).toFixed(2);
             sgst =  parseFloat(item.csgst).toFixed(2);
             igst =  parseFloat(item.cigst).toFixed(2);

            }
           
            // Calculate Cess amount based on remaining quantity
            const cess = (remainingQty * parseFloat(item.cess_rate)).toFixed(2);
        const rowHtml = `
            <tr>
              ...
              <input type="hidden" name="products[${index}][batch_no]" value="${item.batch_no}">
                
                <!-- Conditionally add Batch No column data -->
                <td data-field="batch_no" style="display: ${hasBatchNo ? 'table-cell' : 'none'}">${item.batch_no}</td>

                <td data-field="qty">
                    <span>${parseFloat(remainingQty).toFixed(2)}</span>
                 <input type="hidden" class="form-control" name="products[${index}][qty]" value="${parseFloat(remainingQty).toFixed(2)}" data-original="${parseFloat(remainingQty).toFixed(2)}" >
                </td>
                <td data-field="price">
    <span>${parseFloat(item.price).toFixed(2)}</span>
  
    <input type="hidden" name="products[${index}][price]" value="${parseFloat(item.price).toFixed(2)}" data-original="${parseFloat(item.price).toFixed(2)}">
</td>
                ...
                <td data-field="gst">
                   <span> ${parseFloat(item.gst).toFixed(2)}</span>
                    <input type="hidden" name="products[${index}][gst]" value="${parseFloat(item.gst).toFixed(2)}">
                     <input type="hidden" name="products[${index}][gst_amt]" value="${parseFloat(gstAmt).toFixed(2)}">
                </td>
                <td data-field="cgst">
                   <span>${parseFloat(cgst).toFixed(2)}</span>
                    <input type="hidden" name="products[${index}][cgst]" value="${parseFloat(cgst).toFixed(2)}">
                </td>
                <td data-field="sgst">
                   <span> ${parseFloat(sgst).toFixed(2)}</span>
                    <input type="hidden" name="products[${index}][sgst]" value="${parseFloat(sgst).toFixed(2)}">
                </td>
                <td data-field="igst">
                   <span> ${parseFloat(igst).toFixed(2)}</span>
                    <input type="hidden" name="products[${index}][igst]" value="${parseFloat(igst).toFixed(2)}">
                </td>
                <td data-field="cess_rate">
                   <span> ${parseFloat(item.cess_rate).toFixed(2)}</span>
                    <input type="hidden" name="products[${index}][cess_rate]" value="${parseFloat(item.cess_rate).toFixed(2)}">
                     <input type="hidden" name="products[${index}][cess_amt]" value="${parseFloat(cessAmt).toFixed(2)}">
                </td>
                <td data-field="total">
                   <span> ${parseFloat(gt).toFixed(2)}</span>
                       <input type="hidden" name="products[${index}][total]" value="${parseFloat(gt).toFixed(2)}">
                </td>
                <td>
    ...
    // Populate totals
    $('#final_taxable_amt').text(parseFloat(tottax).toFixed(2));
    $('#final_gst_amount').text(parseFloat(totgstAmt).toFixed(2));
    $('#final_cess_amount').text(parseFloat(totcessamt).toFixed(2));
...
$(document).on('click', '.fa-edit', function () {
        ...
        // Get current values
        const currentQty = parseFloat(row.find('td[data-field="qty"] span').text().trim()) || 0;
        const currentPrice = parseFloat(row.find('td[data-field="price"] span').text().trim()) || 0;

        // Hide spans and show input fields
        row.find('td[data-field="qty"] span, td[data-field="price"] span').hide();
        row.find('td[data-field="qty"]').append(`<input type="number" class="form-control qty-input" value="${currentQty}" min="1">`);
        row.find('td[data-field="price"]').append(`<input type="number" class="form-control price-input" value="${currentPrice}" min="1" step="0.01">`);
...
//     const discount = parseFloat(row.find('td[data-field="discount"] input').val()) || 0;
//     const gstRate = parseFloat(row.find('td[data-field="gst"] input').val()) || 0;
//     const cessRate = parseFloat(row.find('td[data-field="cess_rate"] input').val()) || 0;

//     // Calculate line total, GST, and cess
//     const discountAmt = updatedPrice * discount / 100;
//     const lineTotal = updatedQty * updatedPrice - discountAmt;
...
//     // Calculate total including GST and cess
//     const total = lineTotal + gstAmt + cessAmt;

//     // Update the row with the new values
//     row.find('td[data-field="qty"] span').text(updatedQty.toFixed(2));
//     row.find('td[data-field="price"] span').text(updatedPrice.toFixed(2));
//     row.find('td[data-field="gst"] span').text(gstAmt.toFixed(2));
...
//     row.find('input[name*="[qty]"]').val(updatedQty.toFixed(2));
//     row.find('input[name*="[price]"]').val(updatedPrice.toFixed(2));
//     row.find('input[name*="[gst_amt]"]').val(gstAmt.toFixed(2));
//     row.find('input[name*="[cgst]"]').val(cgstAmt.toFixed(2));
//     row.find('input[name*="[sgst]"]').val(sgstAmt.toFixed(2));
//     row.find('input[name*="[igst]"]').val(igstAmt.toFixed(2));
//     row.find('input[name*="[cess_rate]"]').val(cessAmt.toFixed(2));
...
//     // Update hidden input fields for totals
...
//     // Change the button icon and color back to edit
//     $(this).removeClass('fa-save').addClass('fa-edit').css('color', 'blue');

//     // Recalculate totals for other rows
//     recalculateTotals();
// });
...
//     // Get original values
//     const originalQty = parseFloat(row.find('td[data-field="qty"] input[name*="[qty]"]').attr('data-original')) || 0;
//     const originalPrice = parseFloat(row.find('td[data-field="price"] input[name*="[price]"]').attr('data-original')) || 0;

//     // Get updated values
//     const updatedQty = parseFloat(row.find('td[data-field="qty"] input.qty-input').val()) || 0;
//     const updatedPrice = parseFloat(row.find('td[data-field="price"] input.price-input').val()) || 0;

//     if (updatedQty > originalQty) {
//     Swal.fire({
//         icon: "error",
//         title: "Quantity Exceeded",
//         text: `Quantity (${updatedQty}) exceeds the original invoice quantity (${originalQty}).`,
//         confirmButtonColor: "#d33",
//         confirmButtonText: "OK"
//     }).then(() => {
//         row.find('td[data-field="qty"] input.qty-input').val(originalQty).focus().select();
//     });
//     return;
// }
...
//         text: `Price (${updatedPrice}) exceeds the original invoice price (${originalPrice}).`,
//         confirmButtonColor: "#d33",
//         confirmButtonText: "OK"
//     }).then(() => {
//         row.find('td[data-field="price"] input.price-input').val(originalPrice).focus().select();
//     });
//     return;
// }
...
//     row.find('td[data-field="qty"] input[name*="[qty]"]').val(updatedQty.toFixed(2));
//     row.find('td[data-field="price"] input[name*="[price]"]').val(updatedPrice.toFixed(2));

//     // Remove input fields after saving
//     row.find('.qty-input, .price-input').remove();

//     //  Recalculate and update all values
//     recalculateRow(row);
//     recalculateTotals();
...
if (updatedPrice > originalPrice) {
   ...
   function recalculateRow(row) {
    const qty = parseFloat(row.find('td[data-field="qty"] input[name*="[qty]"]').val()) || 0;
    const price = parseFloat(row.find('td[data-field="price"] input[name*="[price]"]').val()) || 0;
    const discount = parseFloat(row.find('td[data-field="discount"] input[name*="[discount]"]').val()) || 0;
    const gstRate = parseFloat(row.find('td[data-field="gst"] input[name*="[gst]"]').val()) || 0;
    const cessRate = parseFloat(row.find('td[data-field="cess_rate"] input[name*="[cess_rate]"]').val()) || 0;
...
function recalculateTotals() {
    let taxableAmount = 0, totalGstAmount = 0, totalCessAmount = 0, grandTotal = 0;

    $('#item-list tbody tr').each(function () {
        const row = $(this);
        const taxableValue = parseFloat(row.find('td[data-field="qty"] input[name*="[qty]"]').val()) * 
                             parseFloat(row.find('td[data-field="price"] input[name*="[price]"]').val());

        const gstRate = parseFloat(row.find('td[data-field="gst"] input[name*="[gst]"]').val()) || 0;
        const gstAmt = (taxableValue * gstRate) / 100;  // Correct GST Calculation
        const cessAmt = parseFloat(row.find('td[data-field="cess_rate"] input[name*="[cess_amt]"]').val()) || 0;

        taxableAmount += taxableValue;
        totalGstAmount += gstAmt;  // ✅ Properly accumulating GST for all rows
        totalCessAmount += cessAmt;
        grandTotal += taxableValue + gstAmt + cessAmt;
    });

    $('#final_taxable_amt').text(taxableAmount.toFixed(2));
    $('#final_gst_amount').text(totalGstAmount.toFixed(2));  // ✅ Fixing GST sum
    $('#final_cess_amount').text(totalCessAmount.toFixed(2));
    $('#gtotal').text(grandTotal.toFixed(2));
