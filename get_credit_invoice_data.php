...
if (isset($_GET['invoiceID'])) {
    ...
    // Fetch invoice item details excluding products already credited
    // $itemQuery = "
    //     SELECT ii.id, ii.productid, ii.product, 
    //            ii.prod_desc,  ii.qty, 
    //            ii.price, 
    //            ii.discount, 
    //            ii.gst, 
    ...
    //     LEFT JOIN credit_note_items cn ON ii.id = cn.productid
    //     WHERE ii.invoice_id = ? AND cn.productid IS NULL"; // Exclude credited products
   

$itemQuery = "SELECT i.id , i.productid, i.product, i.batch_no, i.prod_desc, i.qty, i.price, i.discount, i.gst, i.cgst, i.sgst, i.igst, i.cess_rate, i.cess_amount, i.total,  IFNULL(SUM(ci.qty), 0) AS credited_qty, ci.cgst as ccgst ,ci.sgst as csgst,ci.igst as cigst
        FROM invoice_items i
        LEFT JOIN credit_note_items ci ON i.productid = ci.productid AND ci.cnote_id IN (SELECT c.id FROM credit_note c WHERE invoice_id = i.invoice_id)
        WHERE i.invoice_id = ? 
