...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    amount = ?, 
    debit_credit = 'D', 
    receipt_or_voucher_no = (SELECT voucher_id FROM voucher WHERE id = ?) 
WHERE voucher_id = ?";

$stmt_ledger = $conn->prepare($ledger_query);
$stmt_ledger->bind_param("sisdii", $transaction_date, $customer_id, $voucherDetails['customerName'], $paid_amount, $voucher_id, $voucher_id);
    ...
    $stmt_reconcile->bind_param("di", $paid_amount, $voucher_id);
    ...
    $invoice_update_query = "UPDATE pi_invoice SET due_amount = ?, status = ? WHERE id = ?";
    $stmt_invoice_update = $conn->prepare($invoice_update_query);
    $stmt_invoice_update->bind_param("dsi", $due_amount, $status, $invoice_id);
