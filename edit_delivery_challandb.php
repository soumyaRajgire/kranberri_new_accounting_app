...
// Debugging log
error_log(print_r($_POST['products'] ?? 'No products data', true), 3, "debug_products.log");

if (isset($_POST['update'])) {
    // Retrieve form data with validation
    $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
    $purchaseNo = isset($_POST['purchaseNo']) ? mysqli_real_escape_string($conn, $_POST['purchaseNo']) : '';
    $purchaseDate = $_POST['purchaseDate'] ?? '';
    $dueDate = $_POST['dueDate'] ?? '';
    $terms_condition = $_POST['terms_condition'] ?? '';
    $note = $_POST['note'] ?? '';
    $pack_price = isset($_POST['pack_price']) ? floatval($_POST['pack_price']) : 0;
    $dc_code = mysqli_real_escape_string($conn, $purchaseNo);
    ...
    $total_amount = floatval(mysqli_real_escape_string($conn, $_POST['total_amount'] ?? '0'));
...
if ($row1 = mysqli_fetch_array($result1)) {
  ...
  $table = array(array("D-Challan Number","$dc_code","Transportation Mode","{$transportMode}"));
$lineheight = 7;
$fontsize = 9;
...
$table = array(array("D-Challan Date","$purchaseDate","Vehicle No.","$vehicleNo"));
$lineheight = 7;
$fontsize = 9;
$widths = array(47.25,47.25,47.25,47.25);
$aligns = array('L','L','L','L');
...
$pdf->SetFillColor(255, 255, 255); // Reset to white background for table content
     ...
     // Delete old invoice_items entries
        $delete_items_sql = "DELETE FROM delivery_challan_items WHERE dc_id=?";
        $stmt_delete = $conn->prepare($delete_items_sql);
        $stmt_delete->bind_param("i", $dc_id);
        $stmt_delete->execute();
...
foreach ($_POST["products"] as $key => $product) {
    ...
    $sql = "INSERT INTO `delivery_challan_items` 
    (`dc_id`, `itemno`, `product_id`, `product`, `prod_desc`, `qty`, `price`, `discount`, `line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
print_r([
    'dc_id' => $dc_id,
    'itemno' => $key,
    'product_id' => $product_id, // Corrected
    'product' => $products,
    'prod_desc' => $proddesc,
    'qty' => $qtyvalue,
    'price' => $priceval,
...
if (!$stmt->execute()) {
    error_log("Insert failed for product $key: " . $stmt->error . "\n", 3, "debug_errors.log");
    die("Execution failed: " . $stmt->error);
} else {
    error_log("Inserted product $key successfully.\n", 3, "debug_success.log");
}
...
$pdf->Cell(30, 6, "D-Challan Total", 'B', 0, 'R');
...
$y = $pdf->GetY();
 ...
 $gtotal = $sub_total + $pack_price + $total_tax;
    ...
    $update_file_query = "UPDATE delivery_challan SET dc_file=?,dc_date=?, due_date=?, total_amount=?, total_gst=?, total_cess=?,grand_total=?, due_amount =?,terms_condition=?, note=?  WHERE id=?";
    $stmt_file = $conn->prepare($update_file_query);
    if (!$stmt_file) {
        die("Error preparing file update statement: " . $conn->error);
    }
    $stmt_file->bind_param("ssssssssssi", $filename, $purchaseDate, $dueDate,$final_taxable_amt, $final_gst_amount, $final_cess_amount, $total_amount, $total_amount, $terms_condition, $note, $dc_id);
    if (!$stmt_file->execute()) {
        die("Error updating D-Challan file: " . $stmt_file->error);
    }
    $stmt_file->close();
}
    echo '<script>alert("Successfully Updated D-Challan");';
