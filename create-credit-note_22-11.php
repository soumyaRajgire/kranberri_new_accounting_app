...
<!-- Status (optional) -->
<input type="hidden" name="status" value="Pending"> <!-- Set default status as Pending -->

<!-- Created By (Session User) -->
<input type="hidden" name="created_by" value="<?php echo $_SESSION['name']; ?>"> <!-- Use session data -->
...
GSTIN: 29AAICK7493G1ZX<br/></h5>
                    ...
                    <div class="py-1 input-group">
                      ...
                      if ($row1 = mysqli_fetch_array($result1)) {
                        $id = $row1['id'] + 1;
                        $credit_note_code = "CNOTE" . str_pad($id, 5, '0', STR_PAD_LEFT);
                      } else {
                        $credit_note_code = "CNOTE00001";
                      }
                      ?>
                      <input class="form-control" type="text" id="creditNoteNo" value="<?php echo $credit_note_code; ?>" name="creditNoteNo" />
                      <label class="form-control col-sm-5" for="creditNoteNo">Credit Note No</label>
                    </div>

                    <!-- credit Note Date -->
                    <div class="py-1 input-group">
                      <input class="form-control" type="date" id="creditNoteDate" name="creditNoteDate" required />
                      <label class="form-control col-sm-5" for="creditNoteDate">Note Date</label>
                    </div>
                    
                    <!-- Due Date -->
                    <div class="py-1 input-group">
                        <select class="form-control" id="purchaseInvoiceDropdown" name="purchaseInvoiceDropdown" onchange="fetchInvoiceData(this.value)">
                            <option value="">Select Purchase Invoice</option>
                            <?php
                            $sql = "SELECT id, pinvoice_code FROM purchase_invoice";
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#customer_name_choice").change(function() {
            ...
            // Check if the selected value is in the expected format
            if (values.length === 2) {
                const customername = values[0];
                const mobileNumber = values[1];

                // Find the customer ID associated with the selected value
                let customerId = null;
                const dataListOptions = document.getElementById('customer_name').options;
                for (let i = 0; i < dataListOptions.length; i++) {
                    if (dataListOptions[i].value === selectedValue) {
                        customerId = dataListOptions[i].getAttribute('data-customerid');
                        break;
                    }
                }

                // Proceed with the AJAX request if customerId is found
                if (customerId) {
                    fetchCustomerData({ customerID: customerId, customername: customername, mobileNumber: mobileNumber });
                }
            } else {
        ...
        // Handle purchase invoice selection from the dropdown
        $('#purchaseInvoiceDropdown').change(function () {
            const invoiceId = $(this).val();
            if (invoiceId) {
                fetchCustomerData({ invoiceID: invoiceId });
            }
        });
        ...
        function fetchCustomerData(params) {
            $.ajax({
                ...
                success: function(data) {
                    $("#customer_dp").hide(); // Hide the customer selection UI if needed
                    $("#customer_data").html(data); // Load the response into the customer_data div
                },
    ...
    // Function to fetch products related to the invoice via AJAX
    function fetchInvoiceProducts(invoiceId) {
        $.ajax({
            url: 'get_invoice_products.php',
            type: "GET",
            ...
            success: function(response) {
                ...
                products.forEach((product) => {
                    const total = parseFloat(product.price) * parseFloat(product.qty);
                    subTotal += total;
            ...
            error: function(xhr, status, error) {
                console.error("An error occurred while fetching invoice products:", error);
            }
        });
    }
    ...
    function calculateGrandTotal() {
        const subTotal = parseFloat($('#sub_total').text()) || 0;
        const additionalPayable = parseFloat($('#pack_price').val()) || 0;
        const grandTotal = subTotal + additionalPayable;
        $('#gtotal').text(grandTotal.toFixed(2));
        $('[name="total_amount"]').val(grandTotal.toFixed(2));
    }

    // Recalculate grand total when additional payable changes
    $('#pack_price').on('input', calculateGrandTotal);
});
    });
</script>
