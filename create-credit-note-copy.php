...
<input type="text" name="business_state" id="business_state" value="<?php echo htmlspecialchars($user['state']); ?>" hidden>
    ...
    $sql = "
        ...
        WHERE cn.invoice_id IS NULL 
        AND i.branch_id='$bid' 
        AND i.invoice_date >= DATE_SUB('$currentDate', INTERVAL 1 YEAR)";
            ...
            <tr>
                ...
                <td data-field="prod_desc">${item.prod_desc}</td>
                 ...
                 <td data-field="qty">
                    ...
                    <input type="hidden" class="form-control" name="products[${index}][qty]" value="${parseFloat(item.qty).toFixed(2)}" data-original="${parseFloat(item.qty).toFixed(2)}" >
                </td>
                <td data-field="price">
...
$(document).on('click', '.fa-save', function () {
    ...
    // Validation: Prevent exceeding the original values
    // if (updatedQty > originalQty) {
    //     alert(`Quantity (${updatedQty}) exceeds the original invoice quantity (${originalQty}).`);
    //     row.find('td[data-field="qty"] input.qty-input').val(originalQty);
    //     return;
    // }
    // if (updatedPrice > originalPrice) {
    //     alert(`Price (${updatedPrice}) exceeds the original invoice price (${originalPrice}).`);
    //     row.find('td[data-field="price"] input.price-input').val(originalPrice);
    //     return;
    // }
...
/**
 * Function to recalculate totals for the entire table
 */
...
//     $('#item-list tbody tr').each(function () {
//         const row = $(this);
//         const lineTotal = parseFloat(row.find('td[data-field="total"] input[name*="[total]"]').val()) || 0;
//         const gstAmt = parseFloat(row.find('td[data-field="gst"] input[name*="[gst_amt]"]').val()) || 0;
//         const cessAmt = parseFloat(row.find('td[data-field="cess_rate"] input[name*="[cess_amt]"]').val()) || 0;

//         taxableAmount += lineTotal - gstAmt - cessAmt;
