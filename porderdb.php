...
$order_code = mysqli_real_escape_string($conn, $_POST['orderNo']);
...
if(($customer_name != "") )
{
 ...
 $result1=mysqli_query($conn,"select id from purchase_order where id=(select max(id) from purchase_order)");
...
$gstamt = number_format(($gstval / 2) * $total / 100, 2, '.', '');
 ...
 // mysqli_query($conn,"INSERT INTO `purchase_order_items` (`id`, `itemno`, `purchase_order_id`, `product_id`, `product`, `prod_desc`, `qty`, `price`, `line_total`, `gst`, `gst_amt`, `total`, `created_by`) VALUES ('$id1', '$itemnum', '$id', '$productid', '$products','$proddesc','$qtyvalue','$priceval', '$line_tot', '$gstval', '$gstamt', '$tot_formatted', '$created_by')");
  ...
  mysqli_query($conn,"INSERT INTO `purchase_order_items` (`id`, `itemno`, `order_id`, `product_id`, `product`, `prod_desc`, `qty`, `price`, `line_total`, `gst`, `gst_amt`, `total`, `created_by`) VALUES ('$id1', '$itemnum', '$id', '$productid', '$products','$proddesc','$qtyvalue','$priceval', '$line_tot', '$gstval', '$gstamt', '$tot', '$created_by')");
...
// Insert into Database for Session Branch
$sql = "INSERT INTO purchase_order 
    (`id`, `order_code`, `order_file`, `customer_id`, `email`, `order_date`, `due_date`, `total_amount`, 
    `total_tax`, `grand_total`, `terms_condition`, `note`, `status`, `created_by`, `branch_id`) 
    VALUES 
    ('$id', '$order_code', '$filename', '$cst_mstr_id', '$customer_email', '$orderDate', '$dueDate', 
    '$sub_total', '0', '$total_amount', '$terms', '$note', 'pending', '$created_by', '$branch_id')";

if ($conn->query($sql)) {
    // Now create orders for all other branches
    $branch_query = "SELECT branch_id FROM add_branch WHERE business_id = '$business_id'";
    $branch_result = $conn->query($branch_query);
    
    if (!$branch_result) {
        // Log and display an error if the query fails
        die("Error: Unable to fetch branches. MySQL Error: " . $conn->error);
    }
    
    if ($branch_result->num_rows > 0) {
        while ($branch = $branch_result->fetch_assoc()) {
            $current_branch_id = $branch['branch_id']; // Updated to match the correct column name
    
            // Skip the session branch (already inserted above)
            if (isset($branch_id) && $branch_id == $current_branch_id) {
                continue;
            }
    
            // Increment ID for new order
            $new_order_id = $id + 1; // Update `id` logic if required
    
            // Insert new purchase order for the current branch
            $new_sql = "INSERT INTO purchase_order 
                (`id`, `order_code`, `order_file`, `customer_id`, `email`, `order_date`, `due_date`, `total_amount`, 
                `total_tax`, `grand_total`, `terms_condition`, `note`, `status`, `created_by`, `branch_id`) 
                VALUES 
                ('$new_order_id', '$order_code', '$filename', '$cst_mstr_id', '$customer_email', '$orderDate', '$dueDate', 
                '$sub_total', '0', '$total_amount', '$terms', '$note', 'pending', '$created_by', '$current_branch_id')";
    
            if (!$conn->query($new_sql)) {
                // Log the error if the query fails
                error_log("Error creating order for branch ID $current_branch_id: " . $conn->error);
            }
        }
    } else {
        // Display an error if no branches are found
        die("No branches found for the given business_id: $business_id");
    ...
    <script>
        ...
        window.location = "purchase_invoices.php?orderCard";
    </script>
    <?php
} else {
    ?>
    ...
    <script>
        alert("Error Creating Purchase Order: <?php echo $conn->error; ?>");
        window.location = "create-purchase-order.php";
    </script>
    <?php
