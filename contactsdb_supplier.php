...
if($_SERVER['REQUEST_METHOD'] === 'POST')
{
	...
	$supplier_registered_name = mysqli_real_escape_string($conn, $_POST['supplier_registered_name']);
...
$created_by = $_SESSION['name'];
	...
	$sqlSupplier = "INSERT INTO customer_master (business_id,branch_id,title, customerName, entityType, mobile, email, gstin, gst_reg_name, business_name, additional_business_name, display_name, phone_no, fax, account_no, account_name, bank_name, account_type,ifsc_code,branch_name, pan, tan, tds_slab_rate, currency, terms_of_payment, reverse_charge, export_or_sez,contact_type,created_by)
                VALUES (?, ?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmtSupplier = $conn->prepare($sqlSupplier);
$customerType = 'Supplier';
// Specify the correct data types for each parameter in the bind_param call
// $stmtSupplier->bind_param("sssssssssssssssssssssssss", $title, $name, $entity_type, $mobile_number, $email, $supplier_gstin, $supplier_registered_name, $business_name, $display_name, $phone_number, $fax, $account_number, $account_name, $bank_name, $account_type, $ifsc_code, $branch_name, $pan, $tan, $tds_slab_rate, $currency, $terms_of_payment, $reverse_charge, $export_type,$customerType, $created_by);
$stmtSupplier->bind_param("iisssssssssssssssssssssssssss", $business_id_feild,$branch_id_feild,$title, $name, $entity_type, $mobile_number, $email, $supplier_gstin, $supplier_registered_name, $business_name, $additional_business_name,	$display_name, $phone_number, $fax, $account_number, $account_name, $bank_name, $account_type, $ifsc_code, $branch_name, $pan, $tan, $tds_slab_rate, $currency, $terms_of_payment, $reverse_charge, $export_type, $customerType, $created_by);


if ($stmtSupplier->execute() === TRUE) {
	echo "test";
 $supplier_id = $stmtSupplier->insert_id;

$sqlAddress = "INSERT INTO address_master (s_address_line1, s_address_line2,s_city,s_Pincode,s_state,s_country,s_branch,s_gstin,b_address_line1,b_address_line2,b_city,b_Pincode,b_state,b_country,b_branch,b_gstin,customer_master_id  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmtAddress = $conn->prepare($sqlAddress);

$stmtAddress->bind_param("ssssssssssssssssi",$ship_address_line1,$ship_address_line2,$ship_city,$ship_pin_code,$ship_state,$ship_country,$ship_branch_name,$ship_gstin,$bill_address_line1, $bill_address_line2, $bill_city, $bill_pin_code, $bill_state, $bill_country, $bill_branch_name, $bill_gstin,$supplier_id);
...
<script>
    ...
    echo "Error inserting supplier: " . $stmtSupplier->error;
}
