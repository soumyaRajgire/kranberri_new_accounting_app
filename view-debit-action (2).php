...
if (isset($_GET['action']) && $_GET['action'] === 'delete_permanent' && isset($_GET['inv_id'])) {
    ...
    try {
         $query = "SELECT purchase_invoice_id, product_id, qty, total_amount 
              FROM debit_note_items 
              WHERE dnote_id = '$inv_id'";
    $result = $conn->query($query);
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['inv_id'])) {
    ...
    if ($conn->query($query)) {
        ...
        echo "<script>alert('Debit note deleted successfully.'); window.location.href='debit_note_list.php';</script>";
    } else {
        echo "<script>alert('Error deleting debit note.'); window.location.href='debit_note_list.php';</script>";
    }
}
...
<div class="btn-group">
    ...
    <a class="dropdown-item" href="edit_debit_note.php?inv_id=<?php echo $inv_id; ?>">Edit Debit Note</a>
    <a class="dropdown-item" 
       href="view_debit_note.php?action=delete&inv_id=<?php echo $inv_id; ?>" 
       onclick="return confirm('Are you sure you want to delete this debit note?');">
       Delete
    </a>
    <a class="dropdown-item text-danger" 
       href="view_debit_note.php?action=delete_permanent&inv_id=<?php echo $inv_id; ?>" 
       onclick="return confirm('Are you sure you want to permanently delete this debit note? This action cannot be undone.');">
       Delete Permanently
    </a>
    ...
    <a href="" class="btn border border-grey" onclick="printPDF('<?php echo $quotationDetails['dnote_file']?>')" data-toggle="tooltip" data-placement="top" title="Print"><i class="fa fa-print text-grey" aria-hidden="true"></i> </a>
    <a href="<?php echo $quotationDetails['dnote_file']?>" target="_blank" class="btn border border-grey" data-toggle="tooltip" data-placement="top" title="Download"><i class="fa fa-download text-grey" aria-hidden="true"></i></a>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const url = "<?php echo $quotationDetails['dnote_file']; ?>"; // Make sure it is echoed as a string
