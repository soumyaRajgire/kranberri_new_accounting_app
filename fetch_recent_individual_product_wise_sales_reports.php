...
$files = array_filter(scandir($directory, SCANDIR_SORT_DESCENDING), function ($file) use ($branch_id) {
    return strpos($file, "Individual_Product_Wise_Sales_Report_Branch_{$branch_id}_") !== false; // Only fetch relevant reports
});
...
if (empty($files)) {
    echo "<p class='text-center text-muted'>No recent individual product-wise sales reports found.</p>";
} else {
    ...
    foreach ($files as $file) {
        if (is_file($directory . $file)) {
            ...
            // Extract date range from the filename using the correct regular expression
            // Updated pattern to match the filename format: 'DD-MM-YYYY_to_DD-MM-YYYY'
            if (preg_match("/Individual_Product_Wise_Sales_Report_Branch_{$branch_id}_(\d{2})-(\d{2})-(\d{4})_to_(\d{2})-(\d{2})-(\d{4})_(\d{14})/", $file, $matches)) {
            ...
            <div class='report-item d-flex align-items-center p-2 border rounded shadow-sm'>
                ...
                <div class='report-details flex-grow-1'>
                    <p class='report-title mb-0'><strong>Individual Product Sales Report</strong></p>
