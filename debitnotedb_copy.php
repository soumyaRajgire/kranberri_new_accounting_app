...
foreach ($_POST['products'] as $productData) {
 ...
 $gst_amount = floatval($productData['gst_amt']);
    ...
    INSERT INTO debit_note_items (
        dnote_id, productid, product, prod_desc, qty, price, line_total, gst, gst_amt, 
        cgst, sgst, igst, cess_rate, cess_amt,discount, total, created_by
    ) VALUES (
        '$did', '$productid','$productName', '$productDescription', '$quantity', '$price', 
...
$sql = "INSERT INTO debit_note (id, dnote_code, dnote_file, purchase_invoice_id, customer_id, branch_id, customer_name, email, dnote_date, total_amount, total_gst_amount, total_cess_amount, adjusted_amount, terms_condition, note, status, created_by) 
        VALUES ('$did', '$dnote_code', '$filename', '$purchase_invoice_id', '$cst_mstr_id', '$branch_id', '$customer_name', '$customer_email', '$dnoteDate', '$total_amount', '$total_gst_amount', '$total_cess_amount', '$adjusted_amount', '$terms', '$note', 'pending', '$created_by')";
if (!$conn->query($sql)) {
 ...
 $sql_ledger = "INSERT INTO `ledger` (`voucher_id`, `transaction_date`, `transaction_type`, `account_id`, `account_name`, `amount`, `debit_credit`, `receipt_or_voucher_no`, `branch_id`) 
 VALUES ($did, '$dnoteDate', 'Debit Note', '$cst_mstr_id', '$customer_name', '$grand_total1', 'D', '$dnote_code', '$branch_id')";
