...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO inventory_master 
        (inventory_type, catlog_type, name, category, company_name, price, in_ex_gst, gst_rate, non_taxable, net_price, hsn_code, units, cess_rate, cess_amt, sku, opening_stock, opening_stockdate, min_stockalert, max_stockalert, description, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    $stmt->bind_param(
        ...
        $hsn_code, $units, $cess_rate, $cess_amount, $sku, $opening_stock, 
        $opening_stockdate, $min_stockalert, $max_stockalert, $description, $created_by, 
        $opening_stock // For update
    );
