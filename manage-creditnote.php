...
$query = "SELECT 
    ...
    cn.created_by, 
    cn.created_at, 
    pi.invoice_code AS document, 
    cm.gstin,
    (SELECT GROUP_CONCAT(cni.product) 
     FROM credit_note_items AS cni 
     WHERE cni.cnote_id = cn.id) AS products
FROM 
    credit_note AS cn
LEFT JOIN 
    invoice AS pi ON cn.invoice_id = pi.id
LEFT JOIN 
    customer_master AS cm ON cn.customer_id = cm.id
WHERE 
    cn.branch_id='$branch_id' AND cn.is_deleted = 0
ORDER BY  
    cn.id DESC
...
<div class="input-group">
        ...
        <?php if (!empty($data)) : ?>
            <?php foreach ($data as $row) : ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($row['dnote_code']); ?><br/><?= htmlspecialchars(date('d/m/Y', strtotime($row['dnote_date']))); ?></td>
                     <td><?= htmlspecialchars($row['dnote_file']); ?><br/><p>Purchase Invoice</p></td>                  
                    <td><?= htmlspecialchars(date('d/m/Y', strtotime($row['dnote_date']))); ?> <br/> <?= htmlspecialchars($row['created_by']); ?></td>
                </tr>
            <?php endforeach; ?>
        <?php else : ?>
            <tr>
 ...
 <div class="row">
                    ...
                    <div class="card-header">
                        ...
                        <!-- <span class="d-block m-t-5">use class <code>table-striped</code> inside table element</span> -->
                         <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 20px;">
                            ...
                            <form class="form-inline" method="POST" action="download_creditnote_monthly.php" style="display: flex; align-items: center; margin-right: 10px;">
                            ...
                            <form class="form-inline" method="POST" action="download_creditnote_range.php" style="display: flex; align-items: center; margin-right: 10px;">
                            ...
                            <a href="create-credit-note.php" class="btn btn-info" style="color: #fff !important;">Create</a>
                        </div>
    ...
    <?php endif; ?>
                    ...
                    <td>
    <a href="view-credit-action.php?inv_id=<?= htmlspecialchars($row['note_id'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
        <?= htmlspecialchars($row['cnote_code'] ?? '', ENT_QUOTES, 'UTF-8'); ?>
    </a><br/>
    <?= !empty($row['cnote_date']) ? htmlspecialchars(date('d-m-Y', strtotime($row['cnote_date'])), ENT_QUOTES, 'UTF-8') : ''; ?>
</td>
                     <td>
                        <?= htmlspecialchars($row['document'] ?? 'N/A'); ?><br />
                        <p>Invoice</p>
                    </td>
                    <td>
                        <span class="">
                        ...
                        <a href="view-credit-action.php?inv_id=<?= htmlspecialchars($row['note_id'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" style="color:green" >View Credit Note</a>
                    </td>
                    ...
                    <td>
                     <a href="<?= $row['cnote_file']; ?>"  class="btn-sm btn btn-primary" download><i class="fa fa-download" ></i></a>
