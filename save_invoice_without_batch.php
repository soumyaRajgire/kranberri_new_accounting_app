...
$totalSum = round(floatval($final_taxable_amt) + floatval($final_gst_amount) + floatval($final_tcs_amount));
...
$this->SetX($leftWidth + 10);
$this->SetFont('Arial', 'B', 8);
$this->Cell($rightWidth, $rowHeight, "TCS Amount:", 1, 0, 'L');
$this->SetFont('Arial', '', 8);
$this->Cell($rightWidth, $rowHeight, "$final_tcs_amount", 1, 1, 'R');
}
...
if($final_tcs_amount > 0){
$this->Cell($bankDetailsWidth, 5, '', 'L', 0, 'L'); // Left border only
$this->Cell($fieldWidth, 5, 'TCS Amount:', '1', 0, 'L'); // Field label with full border
$this->Cell($valueWidth, 5, number_format($final_tcs_amount, 2), '1', 1, 'R'); // Value with full border
}
...
if ($write_result === false) {
 ...
 // Insert main invoice
       $sql = "INSERT INTO `invoice` (`id`, `invoice_code`, `customer_id`, `customer_name`, `customer_email`, `invoice_date`, `due_date`, `total_amount`, `total_gst`, `total_cess`,`tcs_amount`, `grand_total`,`due_amount`, `terms_condition`, `note`, `status`, `branch_id`, `created_by`,`invoice_file_template1`,`invoice_file_template2`,`invoice_file_template3`,`invoice_file_template4` ) 
                VALUES ('$id', '$invoice_code', '$cst_mstr_id', '$customer_name', '$customer_email', '$purchaseDate', '$dueDate', '$final_taxable_amt', '$final_gst_amount', '$final_cess_amount','$final_tcs_amount', '$total_amount', '$total_amount','$terms', '$note', 'pending', '$branch_id', '$created_by','$filename1','$filename2','$filename3','$filename4')";
