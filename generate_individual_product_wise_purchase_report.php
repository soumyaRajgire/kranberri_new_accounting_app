...
$sql = "SELECT 
            i.invoice_code,
            i.customer_name AS seller_name,
...
$filename = "Individual_Product_Wise_Purchase_Report_Branch_{$branch_id}_{$from_date_formatted}_to_{$to_date_formatted}_{$timestamp}." . ($report_type == 'pdf' ? 'pdf' : 'xlsx');
...
if ($report_type == 'pdf') {
    class PDF extends FPDF {
        function Header() {
            $this->SetFont('Arial', 'B', 10); // Reduce font size
            $this->Cell(275, 7, 'Individual Product Wise Purchase Report', 1, 1, 'C');
            $this->Ln(5);
        }
    ...
    $pdf->SetFont('Arial', 'B', 9); // Reduced header font size

    // Updated Table Headers with Adjusted Widths
    $headers = ['S.No', 'Invoice No.', 'Seller Name', 'Invoice Date', 'Qty', 'Rate', 'Taxable Amt', 'CGST', 'SGST', 'IGST', 'CESS', 'Total'];
    ...
    // Table Data
    $pdf->SetFont('Arial', '', 8); // Reduce font size for values
    $serial = 1;

    if (empty($reportData)) {
        $pdf->Cell(array_sum($widths), 7, 'No data found.', 1, 1, 'C');
    } else {
        foreach ($reportData as $row) {
            ...
            $pdf->Cell($widths[2], 7, $row['seller_name'], 1, 0, 'L');
...
elseif ($report_type == 'excel') {
    ...
    // Headers
    $sheet->fromArray([
        ['S.No', 'Invoice No.', 'Seller Name', 'Invoice Date', 'Quantity', 'Rate', 'Taxable Amount', 'CGST', 'SGST', 'IGST', 'CESS', 'Total Amount']
    ], null, 'A1');
