...
$GLOBALS['invoiceFile'] = isset($row['invoice_file']) ? $row['invoice_file'] : '';
...
if (is_array($quotationDetails) && isset($quotationDetails['status'])) {
    if ($quotationDetails['status'] == "paid") {
        ...
        <?php
        if ($quotationDetails['IRNgenerated_status'] == "No") {
        ?>
       
         <!--<a href="#" data-toggle="modal" data-target="#gstCredentialsModal" data-invoice-id="<?php echo $inv_id; ?>"  class="btn border border-grey " data-toggle="modal" -->
         <!--data-placement="top" title="Open Link">E-Invoice</a>-->
         <!-- Trigger Button (E-Invoice link) -->

       
 <a href="#" data-toggle="modal" data-target="#gstCredentialsModal"  id="myBtn" class="btn border border-grey d-none" data-invoice-id="<?php echo $inv_id; ?>" data-toggle="modal" data-placement="top" title="Open Link">E-Invoice</a>

<!-- <a href="#" id="myBtn" data-target="#gstCredentialsModal" data-invoice-id="<?php echo $inv_id; ?>" class="btn border border-grey" data-toggle="modal" data-placement="top" title="Open Link">E-Invoice</a> -->
        ...
        {
            ?>
           <a href="#" data-toggle="modal" data-target="#gstCredentialsModal"   data-irn="<?php echo $quotationDetails['irn_no']; ?>" class="btn border border-grey " data-toggle="tooltip" data-placement="top" title="Open Link">Cancel E-Invoice</a> 
           <?php
        }
       ...
       <?php
        if ($quotationDetails['IRNgenerated_status'] != "Yes") {
              ?>
         <a href="#" data-toggle="modal" data-target="#gstCredentialsModal" data-invoice-id="<?php echo $inv_id; ?>" class="btn border border-grey " data-toggle="tooltip" data-placement="top" title="Open Link" >E-Invoice</a>
    ...
    // Handle the case where $quotationDetails is not an array or doesn't contain 'status'
...
<div class="btn-group">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink" style="margin-right: 55px;">
    <a class="dropdown-item" href="edit_invoice.php?inv_id=<?php echo $inv_id?>">Edit Invoice</a>
...
$queryInvoice = "SELECT grand_total FROM invoice WHERE id = '$inv_id'";
$resultInvoice = $conn->query($queryInvoice);
$invoiceData = $resultInvoice->fetch_assoc();
$totalInvoiceAmount = floatval($invoiceData['grand_total'] ?? 0);

// Fetch total credit note adjustments applied to this invoice
$query2 = "SELECT SUM(total_amount) as total_credit_note FROM credit_note WHERE invoice_id = '$inv_id'";
$result2 = $conn->query($query2);
$creditNoteData = $result2->fetch_assoc();
$totalCreditNote = floatval($creditNoteData['total_credit_note'] ?? 0);

// Fetch total credit notes issued for the customer (for future adjustment)
$query3 = "SELECT SUM(total_amount) as future_credit FROM credit_note WHERE customer_id = (SELECT customer_id FROM invoice WHERE id = '$inv_id') AND invoice_id != '$inv_id' AND status = 'approved'";
$result3 = $conn->query($query3);
$futureCreditData = $result3->fetch_assoc();
$futureCredit = floatval($futureCreditData['future_credit'] ?? 0);

// Fetch total amount paid via receipts
$tpa = 0;
if ($result1->num_rows > 0) {
    $paymentData = $result1->fetch_all(MYSQLI_ASSOC);
    foreach ($paymentData as $payment) {
        $tpa += floatval($payment['paid_amount']);
    }
}

// **Calculate Adjusted Balance**
$balance = $totalInvoiceAmount - ($tpa + $totalCreditNote);

// Handle Negative Balance (Future Credit Adjustment)
$futureAdjustment = 0;
if ($balance < 0) {
    $futureAdjustment = abs($balance); // Extra credit for future use
    $balance = 0; // No balance due
}
?>
...
<div class="col-lg-4 col-md-4 col-sm-4 mt-2">
    <div class="card">
        <div class="card">
            <div class="" style="border-bottom:1px dashed gray">
                <h6 class="p-3">Payment Information</h6>
            </div>
            ...
            <div class="row pl-1 pr-1 ml-0 mr-0" style="border-bottom:1px dashed gray">
                ...
                <div class="col-md-5 p-3">
                    <span style="color:blue"> INR <?= number_format($totalInvoiceAmount, 2) ?></span>
                </div>
            </div>
            ...
            <?php foreach ($paymentData as $payment): ?>
                <div class="row pl-1 pr-1 ml-0 mr-0" style="border-bottom:1px dashed gray">
                    ...
                    <div class="col-md-5 p-3">
                        <span style="color:blue"> INR <?= number_format($payment['paid_amount'], 2) ?></span>
                    </div>
                </div>
            <?php endforeach; ?>

            <?php if ($totalCreditNote > 0): ?>
                <div class="row pl-1 pr-1 ml-0 mr-0" style="border-bottom:1px dashed gray">
                    <div class="col-md-7 p-3">
                        <span>Credit Note Adjusted</span><br/>
                        <span>(Applied against invoice)</span>
                    </div>
                    <div class="col-md-5 p-3">
                        <span style="color:green"> INR <?= number_format($totalCreditNote, 2) ?></span>
                    </div>
                </div>
            <?php endif; ?>

            <?php if ($futureCredit > 0): ?>
                <div class="row pl-1 pr-1 ml-0 mr-0" style="border-bottom:1px dashed gray">
                    <div class="col-md-7 p-3">
                        <span>Future Credit Notes</span><br/>
                        <span>(Available for future invoices)</span>
                    </div>
                    <div class="col-md-5 p-3">
                        <span style="color:purple"> INR <?= number_format($futureCredit, 2) ?></span>
                    </div>
                </div>
            <?php endif; ?>

            <?php if ($futureAdjustment > 0): ?>
                <div class="row pl-1 pr-1 ml-0 mr-0" style="border-bottom:1px dashed gray">
                    <div class="col-md-7 p-3">
                        <span>Future Adjustment</span><br/>
                        <span>(Credit available for next invoices)</span>
                    </div>
                    <div class="col-md-5 p-3">
                        <span style="color:purple"> INR <?= number_format($futureAdjustment, 2) ?></span>
                    </div>
                </div>
            <?php endif; ?>
            ...
            <div class="row pl-3 pr-3">
                <div class="col-md-7 p-3">
                    <?php
                    $lastPaymentDate = $quotationDetails['invoice_date'];
                    $dueDate = date('Y-m-d', strtotime($quotationDetails['due_date'] . ' +1 day')); 
                    $remainingDays = max(0, strtotime($dueDate) - time()) / (60 * 60 * 24);
                    $remainingDays = round($remainingDays);
                    ?>
                    <span>Balanced</span><br/>
                ...
                <div class="col-md-5 p-3">
                    <span style="color:red"> INR <?= number_format($balance, 2) ?></span>
                </div>
            </div>
        </div>
    </div>
