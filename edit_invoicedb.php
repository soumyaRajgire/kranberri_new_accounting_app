...
include("config.php"); 
    ...
    try {
      ...
      echo  $total_amount = floatval(mysqli_real_escape_string($conn, $_POST['total_amount']));
      echo  $note = mysqli_real_escape_string($conn, $_POST['note']);
        echo $invoice_code = mysqli_real_escape_string($conn, $_POST['purchaseNo']);
        echo $dueDate = mysqli_real_escape_string($conn, $_POST['dueDate']);
        echo $purchaseDate = mysqli_real_escape_string($conn, $_POST['purchaseDate']);
...
$itemno++;
        ...
        $l1 = $line_tot + $cgstval + $sgstval + $igstval;
         ...
         $rows[] = [
                    ...
                    $hsn_code, 
                    strval($qtyvalue),
                    $units_val,                            // Unit (pcs, kgs, etc.)
                    number_format($priceval, 2),       // Rate
                    number_format($line_tot, 2),   // Taxable Value
                    "{$gstval}%",                      // GST Rate
                    number_format($l1, 2),   // Total
                ];
...
include("config.php");
    ...
    $this->Cell($signatureBoxWidth, 5, 'For', 0, 1, 'C'); // Title text centered
...
// echo '<script>alert("for pdf creation ' . $productCount . ' products");</script>';
        ...
        if (empty($product['pqty']) || floatval($product['pqty']) <= 0) {
         ...
         if ($product['pqty'] === '' || floatval($product['pqty']) <= 0) {
        continue; // Skip this iteration if quantity is invalid (empty, 0, or 0.00)
    }
    //  if ($product['pproductid'] === '' || 
    //     $product['pname'] === '' || 
    //     $product['pqty'] === '' || 
    //     floatval($product['pqty']) <= 0) {
    //     continue; // Skip this iteration if any of these fields are invalid
    // }
  ...
  // $file_name2 = "INVOICE-template2".$invoice_code.'.pdf';    
            ...
            $pdfdoc11 = $pdf->Output('S');
         
        
        // file_put_contents($filename1, $pdfdoc);
        ...
        if (empty($pdfdoc11)) {
            ...
            file_put_contents($filename1, $pdfdoc11);
            // echo "<script>alert('Invoice TEMPLATE - 1 created successfully ".$filename1."'); </script>";
        }
...
$pdf->AddTotals();
  ...
  $file_name2 = "INVOICE-template2".$invoice_code.'.pdf';      
    ...
    $pdf2doc22 = $pdf->Output('S');
    
    
        $write_result = file_put_contents($filename2, $pdf2doc22);
...
$pdf->InvoiceBody($invoice_code, $transportMode, $purchaseDate, $vehicleNo, $customer_name);
  ...
  // ob_end_clean();
 $file_name3 = "INVOICE-template3".$invoice_code.'.pdf';
      
    $filename3 = "invoice/".$file_name3;
  
    $pdf2doc3 = $pdf->Output('S');    
    
$write_result = file_put_contents($filename3, $pdf2doc3);
if ($write_result === false) {
    echo "<script>alert('Error writing file for Invoice TEMPLATE 2.');</script>";
} else {
    // echo "<script>alert('Invoice TEMPLATE 3 created successfully at ".$filename3."');</script>";
}
...
// ob_end_clean();
        $file_name4 = "INVOICE-template4".$invoice_code.'.pdf';
      
    $filename4 = "invoice/".$file_name4;


    $pdf2doc2 = $pdf->Output('S');
        
        $write_result = file_put_contents($filename4, $pdf2doc2);
        if ($write_result === false) {
            echo "<script>alert('Error writing file for Invoice TEMPLATE 2.');</script>";
        } else {
            // echo "<script>alert('Invoice TEMPLATE 2 created successfully at ".$filename4."');</script>";
        }
...
$sql_get_prev_qty = "SELECT productid, qty FROM invoice_items WHERE invoice_id = ?";
$stmt_get_prev_qty = $conn->prepare($sql_get_prev_qty);
$stmt_get_prev_qty->bind_param("i", $inv_id);
...
// Prepare statement
        ...
        $transaction_type = "Sale-Deleted"; // This can be changed based on the transaction type
        $remark = "Invoice Sale Deleted"; // Can be customized
...
$delete_items_sql = "DELETE FROM invoice_items WHERE invoice_id=?";
 ...
 // Step 1: Fetch previous stock quantity before updating




      
          $sqll = "INSERT INTO `invoice_items` (`invoice_id`, `itemno`,`productid`, `product`, `prod_desc`, `qty`, `price`, `discount`,`line_total`, `gst`, `cgst`, `sgst`, `igst`, `cess_rate`, `cess_amount`, `total`, `in_ex_gst`,`hsn_code`,`units`)   VALUES ('$inv_id', '$itemno', '$productid','$products_name', '$proddesc', '$qtyvalue', '$priceval', '$discountval','$l1', '$gstval', '$cgstval', '$sgstval', '$igstval', '$cessrateval', '$cessamountval', '$totalval', '$in_ex_gst','$hsn_code','$units_val')";
...
if ($qtyvalue != $prev_qty) {
    ...
    if ($qtyvalue > $prev_qty) {
        ...
        // ✅ Case: Increased Quantity (Stock-Out Increases)
        $diff_qty = $qtyvalue - $prev_qty;
   //    echo "<script>alert('Difference between existing quantity $qtyvalue and previous quantity $prev_qty is: $diff_qty');</script>";
   //     echo "Difference between existing quantity $qtyvalue and previous quantity $prev_qty is: $diff_qty";
    ...
    //echo "<script>alert('Difference between  previous quantity $prev_qty and existing quantity $qtyvalue is: $diff_qty');</script>";
  //  echo "Difference between  previous quantity $prev_qty and existing quantity $qtyvalue is: $diff_qty";
 ...
 // Insert main invoice
        $update_file_query = "UPDATE invoice SET invoice_date=?, due_date=?, total_amount=?, total_gst=?, total_cess=?,grand_total=?, due_amount =?,terms_condition=?, note=?, invoice_file_template1=?,invoice_file_template2=?,invoice_file_template3=?,invoice_file_template4=? WHERE id=?";
    $stmt_file = $conn->prepare($update_file_query);
    if (!$stmt_file) {
        die("Error preparing file update statement: " . $conn->error);
    }
    $stmt_file->bind_param("sssssssssssssi", $purchaseDate, $dueDate,$final_taxable_amt, $final_gst_amount, $final_cess_amount, $total_amount, $total_amount, $terms_condition, $note,$filename1,$filename2,$filename3,$filename4, $inv_id);
    if (!$stmt_file->execute()) {
        die("Error updating invoice file: " . $stmt_file->error);
    }
    $stmt_file->close();

  // echo "<script>alert('vefore additional charges  update created successfully');</script>";
...
$sql_other_details = "UPDATE invoice_other_details 
        SET po_number = '" . ($_POST['other_poNumber'] ?? '') . "', 
            ...
            updated_on = NOW() 
        WHERE  invoice_id = '$inv_id'"; // Replace 'your_condition' with the actual condition
if (!$conn->query($sql_other_details)) {
            throw new Exception("Failed to save other details: " . $conn->error);
        }

  // echo "<script>alert('vefore ledger update created successfully');</script>"
