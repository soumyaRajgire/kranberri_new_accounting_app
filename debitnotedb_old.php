   ...
   $pdf->plot_table($widths, $lineheight, $table,$border,$aligns);
    ...
    if ($row1 = mysqli_fetch_array($result1)) {
        ...
        $pdf->plot_table(array(47.25, 47.25, 47.25, 47.25), 9, array(
            array("Debit Note Number", "$dnote_code", "Place of Supply", "{$row1['s_state']}"),
            array("Debit Note Date", "$dnoteDate", "Created By", "$created_by"),
    ...
    $pdf->Cell(153.3, 10, "Description", 1, 0, 'C', 1);
    // $pdf->Cell(14, 10, "GST", 1, 0, 'C', 1);
    // $pdf->Cell(20, 10, "RATE", 1, 0, 'C', 1);
    // $pdf->Cell(12, 10, "QTY", 1, 0, 'C', 1);
    // $pdf->Cell(20, 10, "GST", 1, 0, 'C', 1);
    $pdf->Cell(36, 10, "TOTAL", 1, 1, 'C', 1);

    $conn->begin_transaction();
    try {
        // Insert `debit_note`
        $sql = "INSERT INTO debit_note (id, dnote_code, dnote_file, customer_id, customer_name, email, dnote_date, total_amount, terms_condition, note, status, created_by, branch_id) 
        VALUES ('$id', '$dnote_code', '$filename', '$cst_mstr_id', '$customer_name', '$customer_email', '$dnoteDate', '$total_amount', '$terms', '$note', 'pending', '$created_by', '$branch_id')";
        ...
        foreach ($_POST["products"] as $key => $val) {
            ...
            $table = array(array($slno++, $products . "\n" . $proddesc, $gstval, $priceval, $qtyvalue, $cgstval + $sgstval, $total));
            $pdf->plot_table(array(7, 92, 14, 20, 12, 20, 24), 7, $table, 1, array('C', 'L', 'C', 'C', 'C', 'C', 'C'));

            $conn->query("INSERT INTO debit_note_items (`dnote_id`, `product`, `prod_desc`, `qty`, `price`, `line_total`, `gst`, `gst_amt`, `total`, `created_by`) 
                          VALUES ('$id', '$products','$proddesc','$qtyvalue','$priceval', '$total', '$gstval', '" . ($cgstval + $sgstval) . "', '$total', '$created_by')");
        }
        ...
        $conn->rollback();
        echo "<script>alert('Error creating debit note: " . $e->getMessage() . "');</script>";
    }
}
?>
